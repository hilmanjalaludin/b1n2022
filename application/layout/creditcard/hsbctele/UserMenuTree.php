 ...
 * -------------------------------------------------------*/
	...
	foreach( $g_ListMenu as $MasterName => $arr_master_result )
 {
			...
			foreach( $arr_master_result as $arr_detail_id => $arr_detail_rows )
		{
			...
			echo "<li id=\"". $out->get_value('id') ."\" > ".
						"<a href=\"javascript:void(0);\"  onclick=\"Ext.ShowMenu('". $out->get_value('file_name') ."','". $out->get_value('menu') ."');\" >".
	...
	echo "<li class=\"dropdown\">
			...
			<ul class=\"dropdown-menu dropdown-user\">
				...
				<li><a href=\"javascript:void(0);\" onclick=\"Ext.ShowMenu('ModBroadcastMsg','Broadcast Messages');\"><i class=\"fa fa-comment fa-fw\"></i> Broadcast Message</a></li>
