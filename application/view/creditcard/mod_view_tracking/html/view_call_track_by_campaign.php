...
{ 
	function _select_report_campaign_id()
 {
	...
	if(($out->field('CampaignStatus')==1 || $out->field('CampaignStatus')==0) && $out->field('CampaignId')=="all"){
		$sql .= sprintf(" where a.CampaignStatusFlag in (%s)",$out->field('CampaignStatus'));
	} else {
		$sql .= sprintf(" and a.CampaignStatusFlag in (%s)",$out->field('CampaignStatus'));
	}

	if($out->field('ProductId')!="all" && $out->field('CampaignId')=="all" && $out->field('CampaignStatus')==2){
		$sql .= sprintf(" where r.ProductId in (%s)",$out->field('ProductId'));
	} else {
		$sql .= sprintf(" and r.ProductId in (%s)",$out->field('ProductId'));
	}
