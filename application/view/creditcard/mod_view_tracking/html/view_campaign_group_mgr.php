...
// model terlebih dahulu.
 ...
 if( !function_exists('_select_report_mgr_value') ) { 
	...
	$sql[] = sprintf("select  a.UserId, a.id, a.full_name from tms_agent a
					  where a.handling_type = %d", USER_GENERAL_MANAGER);
 ...
 */	
 ...
 function _select_campaign_group_mgr_summary( $ManagerId = 0 ) {
...
// get data init freq_call per lead 
	...
	$sql = sprintf("SELECT COUNT(*) as tot_init_call, b.DM_CampaignId as CampaignId,
				...
				left join t_gn_assignment d on a.CustomerId=d.AssignCustId
				where a.HistoryType = 0
				and d.AssignAmgr=%d
				and a.CallHistoryCreatedTs>='%s 00:00:00'
				and a.CallHistoryCreatedTs<='%s 23:59:00'
				group by CampaignId",   $ManagerId, 
										$StartDate, 
										$EndDate  );
// echo $sql;
...
// untuk mendapakan semua data dari masing2 campaign.
	...
	$sql = sprintf("SELECT 
					a.DM_CampaignId as CampaignId, b.AssignAmgr as ManagerId,
					...
					where b.AssignAmgr=%d
					and a.DM_UpdatedTs>='%s 00:00:00'
					and a.DM_UpdatedTs<='%s 23:59:59'		
					group by CampaignId", 	$ManagerId, 
											$StartDate, 
											$EndDate  );
...
// get result data of manager from DB 	
	$result_manager = _select_report_mgr_value(); 
...
// switch data to process OK 	
	...
	switch( $URI->field('interval') )
	{
		...
		if( is_array($result_manager) ) 
		foreach( $result_manager as $ManagerId => $ManagerName ){
			printf("<p class='font-size12 font-bold'>%s</p>", $ManagerName);
			call_user_func( '_select_campaign_group_mgr_summary', $ManagerId );
		}
		break;
 ...
 function _showheaders()
{
	...
	echo "<div class=\"center\">".
		 "<p class=\"normal font-size22\">Report Call Tracking - Summary By Campaign Per MGR</p>".
...
<html>
	<head>
		<title><?php echo title_header("Call Tracking - Summary By Campaign Per MGR");?></title>
