...
sum(if(ass.AssignAmgr,1,0)) 	AS AssignAmgr
	...
	$sql = sprintf("SELECT a.AssignCustId, a.AssignAdmin, a.AssignAmgr, a.AssignMgr, a.AssignMgr, COUNT(a.AssignSpv) AS AssignSpv,
					a.AssignSelerId,b.DM_CampaignId AS CampaignId 
					FROM t_gn_assignment a
					INNER JOIN t_gn_customer_master b ON a.AssignCustId = b.DM_Id
					
					where 1=1
					AND a.AssignSpv != 0
					AND a.AssignSelerId = 0
					and  a.AssignDate >= '%s 00:00:00' and  a.AssignDate <= '%s 23:59:59'", 
										$StartDate, 
										$EndDate,
										$StartDate, 
										$EndDate  );
...
sum(if(ass.AssignSelerId,1,0)) AS AssignSelerId
	...
	sum(if(ass.AssignAmgr,1,0))  AS amgr_kuy
		...
		// edit baru new
	$sql = sprintf("SELECT COUNT(b.DM_Id) AS DataNew,
	a.AssignSelerId,b.DM_CampaignId AS CampaignId
		...
		$sql .=	" group by b.DM_CampaignId";
		// printf(" Data DB NEW QRY 3 edit hilman DataNew: <pre>%s</pre><hr>", $sql);
		
	
		$qry  = $CI->db->query( $sql );
		if( $qry && $qry->num_rows() > 0) 
		foreach( $qry->result_assoc() as $row )  {
			$arr_val_data[$row['CampaignId']]['DataNew'] += $row['DataNew'];	
...
// hitung data size && Utilize
	...
	if(_get_session('HandlingType') == 9){
		$totDataShortAssignAdmin 		= 0;
		$totDataShortAssignMgr 		= 0;
	}else{
	...
	$totDataSolicitedNewDataBaru 	= $val_data['DataNew'];
