...
var navigation = {
	custnav	 : Ext.DOM.INDEX +'/CtiPBX/index/',
	custlist : Ext.DOM.INDEX +'/CtiPBX/content/',
}
...
Ext.DOM.Delete = function()
{
	...
	if(Ext.Cmp('chk_skill').empty()!=true)
	{
		...
		{
			Ext.Ajax({
				url 	: Ext.DOM.INDEX+'/CtiPBX/Delete/',
				method 	:'POST',
				param 	: { Id : Id },
				ERROR : function(e){
					var ERR = JSON.parse(e.target.responseText);
					if( ERR.success ) {
						Ext.Msg("Delete CC PBX ").Success();
						Ext.EQuery.postContent();
					}
					else{
						Ext.Msg("Delete CC PBX").Failed();
						return false;
					}
				}
			}).post();
