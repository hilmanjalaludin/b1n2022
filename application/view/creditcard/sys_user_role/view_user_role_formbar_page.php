...
if( !function_exists('spiner_call_back_add_header') ) 
{
	function spiner_call_back_add_header()
 {
	...
	if( isset($MenuRoleAction['formbars']) ){
		$arr_toolbar = $MenuRoleAction['formbars'];
	}
	
	if( isset($MenuRoleActionUser['formbars']) ){
		$arr_userbar = $MenuRoleActionUser['formbars'];
	}
	...
	if(@in_array($args[0], $arr_insect ))
	{
		...
		return form()->checkbox("RoleFormbarId", null, join("_", $args), array("change" => "window.SetUserFormbars({'act_id': $args[0], 'mnu_id' : $args[1], 'obj_id' : this });"), $btn_disable);
	
	} else {
		return form()->checkbox("RoleFormbarId", null, join("_", $args), NULL, array('disabled' => true));
	}
  }
...
//-------------------- set table attribute -------------------------------
 ...
 $spiner->set_func_page_table("PageFormbar");
