...
var navigation = {
	custnav	 : Ext.DOM.INDEX +'/CtiIVR/index/',
	custlist : Ext.DOM.INDEX +'/CtiIVR/content/',
}
...
Ext.DOM.Delete = function()
{
	...
	if(Ext.Cmp('chk_skill').empty()!=true)
	{
		...
		{
			Ext.Ajax({
				url 	: Ext.DOM.INDEX+'/CtiIVR/Delete/',
				method 	:'POST',
				param 	: { Id : Id },
				ERROR : function(e){
					var ERR = JSON.parse(e.target.responseText);
					if( ERR.success ) {
						Ext.Msg("Delete CC IVR ").Success();
						Ext.EQuery.postContent();
					}
					else{
						Ext.Msg("Delete CC IVR").Failed();
						return false;
					}
				}
			}).post();
