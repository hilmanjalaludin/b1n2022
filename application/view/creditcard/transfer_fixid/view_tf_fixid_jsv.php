<script>
  ...
  window.ViewCmData = function(item) {
    ...
    $('#ui-widget-cm-list').Spiner({
      url: new Array('TransferFixId', 'PageCm'),
  ...
  Ext.DOM.ActionCheckCm = function(item) {
    if (Ext.Cmp(item).getValue() != '' || Ext.Cmp(item).getValue() != null) {
      Ext.Ajax({
        url: Ext.EventUrl(['TransferFixId', 'getCmDetail']).Apply(),
        method: 'POST',
        param: {
          'custid': Ext.Cmp(item).getValue()
        },
        ERROR: function(e) {
          Ext.Util(e).proc(function(response) {
            ...
            Ext.Cmp('campaign_old').setValue(response.data.DM_CampaignId)
            Ext.Cmp('seller_old').setValue(response.data.DM_SellerId)
  ...
  window.UpdateCm = function() {
    ...
    Ext.Ajax({
      url: Ext.EventUrl(['TransferFixId', 'updateCm']).Apply(),
      method: 'POST',
  ...
  Ext.DOM.pickSpv = function(spv) {
    Ext.Ajax({
      url: Ext.EventUrl(['TransferFixId', 'getAgent']).Apply(),
      method: 'POST',
  ...
  Ext.DOM.UpdateCmAssignment = function() {
    ...
    Ext.Ajax({
      url: Ext.EventUrl(['TransferFixId', 'updateCmAssignment']).Apply(),
      method: 'POST',
      param 	: Ext.Join( Data ).object(),
      ERROR: function(e) {
        Ext.Util(e).proc(function(response) {
  ...
  window.ViewCvData = function(item) {
    ...
    $('#ui-widget-cv-list').Spiner({
      url: new Array('TransferFixId', 'PageCv'),
  ...
  Ext.DOM.ActionCheckCv = function(item) {
    ...
    if (Ext.Cmp(item).getValue() != '' || Ext.Cmp(item).getValue() != null) {
      Ext.Ajax({
        url: Ext.EventUrl(['TransferFixId', 'getCvDetail']).Apply(),
        method: 'POST',
        param: {
  ...
  window.UpdateCv = function() {
    ...
    Ext.Ajax({
      url: Ext.EventUrl(['TransferFixId', 'updateCv']).Apply(),
      method: 'POST',
      param 	: Ext.Join( Data ).object(),
      ERROR: function(e) {
        Ext.Util(e).proc(function(response) {
