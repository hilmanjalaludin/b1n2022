...
window.SearchTransferCampaign =function()  {	
	new window.ViewTransferCampaignData ({  orderby : '',   type : '',  page : 0 });	
}

Ext.DOM.ClearDataTransferCampaign = function(){
	Ext.Serialize('formTransferCampaignFilter').Clear(new Array('transfer_campaign_record_page'));
}

window.ViewTransferCampaignData = function( obj )
{
  var formTransferCampaignFilter = Ext.Serialize('formTransferCampaignFilter');
	$('#ui-widget-transfer-campaign-list').Spiner 
	({
		url 	: new Array('MgtAssignment','PageTransferCampaign'),
		param 	: Ext.Join( new Array( formTransferCampaignFilter.getElement()  )).object(),
		order : {
			order_type : obj.type,
			order_by   : obj.orderby,
			order_page : obj.page	
		}, 
		handler : 'ViewTransferCampaignData',
		complete : function( obj ){
			
			// get total to set on right of page 
			this.dataSize = parseInt( $('#ui-total-ui-pager-transfer-campaign-data-record').text() );
			
			// tambahan slider untuk wrap table 
			// add customize for scroll testing 
		
			if( !$('.ui-widget-scroll-top-table-transfer-campaign').length ){
				$("<div class='ui-widget-scroll-top-table-transfer-campaign'>"+
					"<div class='ui-wdget-slider-top-transfer-campaign' style='width:99%;'></div>").insertBefore(obj);
			}
		  // callculate of height this 
		if( $('#ui-pager-transfer-campaign-data').innerHeight() == 0 ){
			...
			this.protectedHeights = (($('.ui-fieldset-transfer-campaign-top').innerHeight()+$('.ui-fieldset-transfer-campaign-bottom').innerHeight()) - 55);
			this.protectedWidths  = $(window).innerWidth() - ($(window).innerWidth()/3);
		}
		  console.log(this.protectedHeights);
		
		 // replace styleSheets of overflow method 	
			$(".ui-widget-transfer-campaign-list").css({  
				width: Math.round(this.protectedWidths), 
				height : this.protectedHeights  
			});
			$(".ui-widget-transfer-campaign-list").addClass("ui-scroller-width-right-border-fixed");
			
			
			if( this.dataSize == 0 ){
				this.maxWidthPager = (($('.ui-fieldset-transfer-campaign-top').innerHeight()+$('.ui-fieldset-transfer-campaign-bottom').innerHeight()) - 55); 
				$(".ui-widget-transfer-campaign-list").css({   
			...
			if( window.percentData.transfer ){
				$('.ui-widget-transfer-campaign-list').scrollLeft( window.percentData.transfer );
			}	
			...
			$(".ui-wdget-slider-top-trans").slider({
				slide: function( event, ui ) {
					this.widthData = $('.ui-pager-transfer-campaign-data-headers').innerWidth();
			...
			gTotalData.transfer_campaign = this.dataSize;
			// components attributes process 
			Ext.Cmp('transfer_campaign_user_quantity').setValue(0);
			Ext.Cmp('transfer_campaign_user_total').setValue(this.dataSize);
			Ext.Cmp('transfer_campaign_user_total').disabled(true);
		}
	});		
}

window.SubmitTransferCampaign = function()
{
 var frmTransferCampaignFilter = Ext.Serialize("formTransferCampaignFilter"),
 		frmTransferCampaignOption = Ext.Serialize("frmTransferCampaignOption"),
	 frmTransferCampaignGrid = new Array();
	
	
// kecuali dta yang berikut ini.
 var dataNotRequired = new Array('transfer_campaign_check_kuota');
 
 if( !frmTransferCampaignOption.Complete(dataNotRequired) )	{
	Ext.Msg('Form Option Not Complete !').Info();
	return false;
 }
 
 frmTransferCampaignGrid['DM_CampaignId'] = Ext.Cmp('DM_CampaignId').getValue();
 
 var Data = new Array(
	frmTransferCampaignFilter.getElement(), 
	frmTransferCampaignOption.getElement(),
	frmTransferCampaignGrid
  );
  
   Ext.Ajax  ({
	 url 	 : Ext.EventUrl( new Array('UserTransferCampaign', 'index')).Apply(),
	 method  : 'POST',
	 ...
	 success : function( xhr )  {
		...
		Ext.Util( xhr ).proc(function( row ){
			if( row.success ) {
				var dataMsg = null	
				dataMsg = window.sprintf("success customer : < %s > \n duplicate custno : < %s > \n success customer verif : < %s > \n duplicate custno verif : < %s >\n",
													row.report.successCustomer,
													row.report.failedCustomer,
													row.report.successCustomerVerif,
													row.report.failedCustomerVerif);
				
			// return message / callMessageUser OK 	
					Ext.Msg( dataMsg ).Info();
					new Ext.DOM.SearchTransferCampaign();
			}
			else {
				// return false OK 
...
Ext.DOM.ActionCheckTransferCampaign = function( obj ) 
 {
	if( obj.checked && obj.value == 2 )
	{
		var BoxList = Ext.Cmp("DM_CampaignId").getValue()
		if( BoxList.length == 0 ){
			Ext.Cmp('DM_CampaignId').setChecked();
			BoxList = Ext.Cmp("DM_CampaignId").getValue();
		}
		
		Ext.Cmp('transfer_campaign_user_quantity').setValue(0);
		Ext.Cmp('transfer_campaign_user_total').setValue(BoxList.length);
		Ext.Cmp('transfer_campaign_user_total').disabled(true);
			
	} else {
		Ext.Cmp('transfer_campaign_user_quantity').setValue(0);
		Ext.Cmp('transfer_campaign_user_total').setValue(gTotalData.transfer_campaign);
		Ext.Cmp('transfer_campaign_user_total').disabled(true);
	}	
 }
 /*
