 ...
 **/
Ext.DOM.navigation = {
	custnav	: Ext.DOM.INDEX+'/Wallboards/index',
	custlist: Ext.DOM.INDEX+'/Wallboards/Content'
}
...
var daily_target = Ext.Cmp('daily_today').getValue(),
	...
	else
	{
		Ext.Ajax
		({
			url 	: Ext.DOM.INDEX+'/Wallboards/SaveAdd',
			method 	: 'POST',
			param 	: {
			...
			ERROR : function(e){
				...
				if( ERR.success==1 ){
					alert('Success, Add Wallboards');
					Ext.EQuery.postContent();	
				}
				else{
					alert('Failed, Add Wallboards');
				}
			}
		}).post();
	}
...
Ext.DOM.addCampaign = function()
{
	Ext.Ajax
	({
		url 	: Ext.DOM.INDEX+'/Wallboards/AddTpl',
		method 	: 'GET',
		param 	: {
 ...
 **/
 Ext.DOM.UpdateCores = function()
 {
	...
	if( Ext.Cmp('daily_today').empty()) { 
		...
		Ext.Ajax
		({
			url		: Ext.DOM.INDEX+'/Wallboards/Update',
			method 	: 'POST',
			param 	: {
				daily_today: daily_target,
				mtd_h1 :mtd_h1,
...
{	
 ...
 console.log(id);
	if( Ext.Cmp('cmp_check_list').empty()!=true){
		...
		{
			Ext.Ajax({ 
				url		: Ext.DOM.INDEX+'/Wallboards/EditTpl',
				method  : 'post',
				param 	: {
					id : id,
					time : Ext.Date().getDuration()
...
Ext.DOM.Delete = function()
{
 ...
 {
	Ext.Ajax({
		url 	: Ext.DOM.INDEX+'/Wallboards/Delete',
		method 	: 'POST',
		param 	: {
