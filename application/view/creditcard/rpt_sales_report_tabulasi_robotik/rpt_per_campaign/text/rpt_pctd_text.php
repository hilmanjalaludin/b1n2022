...
foreach ($data->result_record() as $row) {
	...
	$writefix->write_content($i, 1, $row->field('MERCHANT_ID'));
	// if ($row->field('TX_Usg_Tenor') == '3') {
	// 	$writefix->write_content($i, 1, 0);
	// } elseif ($row->field('TX_Usg_Tenor') == '6') {
	// 	$writefix->write_content($i, 1, 'PROG INST KK 0.8%');
	// } elseif ($row->field('TX_Usg_Tenor') == '12') {
	// 	$writefix->write_content($i, 1,  'PROG INST KK 0.8%');
	// } elseif ($row->field('TX_Usg_Tenor') == '18') {
	// 	$writefix->write_content($i, 1, 'PROG INST KK 0.75 %');
	// } elseif ($row->field('TX_Usg_Tenor') == '24') {
	// 	$writefix->write_content($i, 1, 'PROG INST KK 0.7%');
	// } elseif ($row->field('TX_Usg_Tenor') == '36') {
	// 	$writefix->write_content($i, 1, 'PROG INST KK 0.65%');
	// }
	$writefix->write_content($i, 2, $row->field('AMOUNT'));
	$writefix->write_content($i, 3, $row->field('TGL_TRANSAKSI', 'datestring'));
	...
	$writefix->write_content($i, 5,   $row->field('REF_ID'));
	// $writefix->write_content( $i, 6, '888888888888' );
	$writefix->write_content($i, 6, $row->field('TX_Usg_UpdatedTs', 'datestring'));
