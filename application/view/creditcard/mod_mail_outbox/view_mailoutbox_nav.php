...
var Role = new Ext.Role("MailOutbox");
	Role.extend([
		{ title: " ", icon :"", event: "", key :'MailOutboxId' } // if you have other extends event 
	]);
...
{
  var MailOutboxId = Role.getValue();
  if( MailOutboxId.length ==0 ) {
	Ext.Msg("Please select a rows").Info();
	return false; }
   ...
   ({
		...
		ERROR : function( fn )
		{
			Ext.Util(fn).proc( function( response ){
				...
				if( response.success ){
					Ext.Msg("Delete Mail Outbox").Success();
					Ext.Progress("load_images_id").start();
					Ext.EQuery.postContent();
				} else {
					Ext.Msg("Delete Mail Outbox").Failed();
				}
		    });
		}	
   }).post();
