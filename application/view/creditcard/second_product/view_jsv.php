<script>
	...
	Ext.DOM.RoleBack = function() {
		if (Ext.Msg('Are you sure ?').Confirm()) {
			new Ext.ShowMenu("SecondProduct", Ext.System.view_file_name());
	...
	Ext.DOM.UpdateLayout = function() {
		...
		if (Ext.Msg('Are you sure ?').Confirm()) {
			...
			Ext.Ajax({
				url: Ext.DOM.INDEX + '/SecondProduct/Update/',
				method: 'POST',
				param: {
					CV_Data_Id: Ext.Cmp('CV_Data_Id').getValue(),
					CV_Data_Custno: Ext.Cmp('CV_Data_Custno').getValue(),
					Old_CampaignCode: Ext.Cmp('Old_CampaignCode').getValue(),
					New_CampaignCode: Ext.Cmp('New_CampaignCode').getValue(),
					Old_CV_Data_CustId: Ext.Cmp('Old_CV_Data_CustId').getValue(),
					New_CV_Data_CustId: Ext.Cmp('New_CV_Data_CustId').getValue(),
				},
				ERROR: function(fn) {
					var ERR = JSON.parse(fn.target.responseText);
					if (ERR.success) {
						Ext.Msg("Update Second Product").Success();
					} else {
						Ext.Msg("Update Second Product").Failed();
					}
				}
			}).post();
		}
