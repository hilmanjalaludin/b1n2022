<?php
	...
	Class M_RptSumOutputPctdAprv Extends EUI_Model
	{
		...
		Public Function getData()
		{
			if($_REQUEST['supervisor']) {
				...
				$sql = "select xs.TX_Usg_SellerId, count(xs.TX_Usg_Id) TotalBalcon, SUM(xs.AMOUNT) Amount
							from t_gn_customer_master cs
							inner join tms_agent a on cs.DM_SellerId=a.UserId
							inner join tms_agent b on a.spv_id=b.UserId
							inner  join t_gn_frm_pctd xs on cs.DM_Id=xs.TX_Usg_CustId
						WHERE cs.DM_CallCategoryKode = 'APRV'
							and cs.DM_CallSellerUpdateTs between '".$_REQUEST['start_date']." 00:00:00' and '".$_REQUEST['end_date']." 23:59:59'
							And xs.TX_Usg_SpvKode = '".$_REQUEST['supervisor']."'
