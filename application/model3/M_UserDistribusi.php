...
/**
  ...
  * @return [type]             [description]
  */
 function _get_message_process(){
	 if( is_array( $this->totalProcessed ) ){
...
function _get_row_kuota_user( $UserId = 0, $Username = null ){
	...
	if(  $qry && $qry->num_rows() > 0 ) {
		...
		if( is_object( $row ) ) {	
			...
			$this->totalProcessed[$Username]['kuota']  = $kuota_data;
			$this->totalProcessed[$Username]['limit']  = $kuota_size;
			$this->totalProcessed[$Username]['sisa']   = ($kuota_size-$kuota_data);
...
// difilter .
  $Username = $Usr->get_value('Username');
  
  $this->totalProcessed[$Username] = array( 'total'	  => 0, 
											'failed'  => 0, 
											'success' => 0 );
 ...
 {
	$this->totalProcessed[$Username]['total']+=1;
	 
  // cek kuota data jika data berisi true maka process 
   $this->kuota = $this->_get_row_kuota_user( $Usr->get_value('UserId'), $Username);
   if( is_bool($this->kuota) and !$this->kuota ){
	   $this->totalProcessed[$Username]['failed'] +=1;
	   continue;
   }	
...
// jika success update ke table di bawah ini.
	...
	if( $this->db->update( 't_gn_assignment' ) )  {
		...
		// process 
		$this->totalProcessed[$Username]['success'] +=1;
		$this->callTotalData++;
	}	
 }
  
// distribusi ke level  "USER_MANAGER" dengan data yang 
// difilter .
...
// reset data query process 
	...
	if( $this->db->update('t_gn_assignment') ) {
		$this->_set_row_kuota_user($Usr->get_value('UserId'), $Username); // update kuota 
		$this->_set_row_update_master( $Usr, $this->callDisAsgID);
		$this->_event_loger_distribute( $this->callDisAsgID, 'DIS');
...
/**
 ...
 function _set_row_distribusi_rata( $out = null ) {
