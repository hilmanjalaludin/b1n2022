...
class M_NewDashboard extends EUI_Model
{
	...
	{
		$this->_start_date 	= (_get_have_post('dsb_start')?date('Y-m-d 00:00:00',strtotime(_get_post('dsb_start'))):date('Y-m-d 00:00:00'));
		$this->_end_date 	= (_get_have_post('dsb_end')?date('Y-m-d 23:59:59',strtotime(_get_post('dsb_end'))):date('Y-m-d 23:59:59'));
	}
	...
	function _get_combo_user_query( $where=array() )
	{
		...
		$sql = "select UserId, id from tms_agent
				where 1=1
				AND user_state = 1 ";
	...
	function _query_solicited_tso($param)
	{
		...
		$sql = "select b.UserId, b.id, b.tl_id, count(a.CustomerId) as solicited 
				from (select ax.AgentCode, ax.CustomerId from t_gn_callhistory ax
							where 1=1
	...
	function _query_solicited_spv($param)
	{
		...
		$sql = "select b.UserId, b.id, b.tl_id, count(a.CustomerId) as solicited 
				from (select ax.SPVCode, ax.CustomerId from t_gn_callhistory ax
							where 1=1
							and date(ax.CallHistoryCallDate) >= '{$this->_start_date}'
							and date(ax.CallHistoryCallDate) <= '{$this->_end_date}'
							and ax.HistoryType = 0
	...
	function _query_solicited_atm($param)
	{
		...
		$sql = "select b.UserId, b.id, b.tl_id, count(a.CustomerId) as solicited 
				from (select ax.ATMCode, ax.CustomerId from t_gn_callhistory ax 
							where 1=1
							and date(ax.CallHistoryCallDate) >= '{$this->_start_date}'
							and date(ax.CallHistoryCallDate) <= '{$this->_end_date}'
							and ax.HistoryType = 0
	...
	function _query_solicited_mgr($param)
	{
		...
		$sql = "select b.UserId, b.id, b.tl_id, count(a.CustomerId) as solicited 
				from (select ax.AMGRCode, ax.CustomerId from t_gn_callhistory ax
							where 1=1
							and date(ax.CallHistoryCallDate) >= '{$this->_start_date}'
							and date(ax.CallHistoryCallDate) <= '{$this->_end_date}'
							and ax.HistoryType = 0
