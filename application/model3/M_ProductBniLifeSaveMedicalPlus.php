...
// ------- saving data --------------------------------
	...
	$this->db->set("PayerOccupationId", $out->get_value('PayerOccupationId','strval'));
...
// ------------- jika duplikasidata  -----------------------------------------------------------
	...
	$this->db->duplicate("PayerOccupationId", $out->get_value('PayerOccupationId','strval'));
...
// =============================================================================================
	...
	// --- policy created ----------------------	
		...
		// ------------ this condition for premi type of plan  --------------------------
			...
			{	
				if( in_array($GroupPremi, array($ProductMaxGroup) ) ) {
		...
		// ----- save data insured  -------------------- 
		 if( $PolicyId ) 
		 {
			...
			(
					...
					"InsuredAge" 		 => $out->get_value( join( "_", array("InsuredAge",$GroupPremi)), 'strval'),
		...
		// ----------------- save data payer untuk product bni life Active  ------------------------------------------------------
		  ...
		  {	
			 ...
			 (	
				...
				'PayerOccupationId'				=> $out->get_value('PayerOccupationId','strtoupper'),	
		...
		// ----- save data insured  -------------------- 
		 if( $PolicyId ) 
		 {
			...
			(
					...
					"InsuredDOB" 		 => $out->get_value( join( "_", array("InsuredDOB",$GroupPremi)), '_getDateEnglish'),
					"InsuredAge" 		 => $out->get_value( join( "_", array("InsuredAge",$GroupPremi)), 'strval'),
					"RelationshipTypeId" => $out->get_value( join( "_", array("RelationshipTypeId", $GroupPremi)), 'strval'),
					"InsuredFirstName"   => $out->get_value( join( "_", array("InsuredFirstName", $GroupPremi)), 'strtoupper'),
					"InsuredLastName" 	 => $out->get_value( join( "_", array("InsuredLastName", $GroupPremi )), 'strtoupper'),
					"CreatedById" 		 => _get_session('UserId'),
					"UpdatedById" 		 => _get_session('UserId'),
