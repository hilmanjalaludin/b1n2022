<?php
  ...
  */
class M_SrcCustomerListAutoDial extends EUI_Model
{
...
//  ADMIN / ROOT 		
	...
	if( $cok->cookie(array(USER_ROOT)) ){
		$this->EUI_Page->_setWhereNotIn("COALESCE(a.DM_LastCategoryId,0)", array(APRV,CLOS,BLCK ));
		// $this->EUI_Page->_setWhereNotIn("COALESCE(a.DM_LastCategoryId,0)", array(APRV,CLOS,YCOM,NCOM,BLCK ));
...
// SPV -- 
	if( $cok->cookie(USER_SUPERVISOR) ){ 
		...
		//add date expired
		#$this->EUI_Page->_setAnd("date(a.DM_DateExpired) >= curdate()");
		$this->EUI_Page->_setAnd("date(c.CampaignEndDate) >= curdate()");
		$this->EUI_Page->_setAnd("b.AssignSpv", $cok->field('UserId'));
	}
...
// -- AGENT ( TSR )	-- 
// edit hilman and andi RDPC 05112020
	if( $cok->cookie(array(USER_AGENT_OUTBOUND, USER_AGENT_INBOUND))){ 
		$this->EUI_Page->_setWhereNotIn("COALESCE(a.DM_LastCategoryId,0)", array(4, APRV,CLOS,BLCK, 13, 700 ));
		// $this->EUI_Page->_setWhereNotIn("COALESCE(a.DM_LastCategoryId,0)", array(NOCT, DECL, APRV,CLOS,YCOM,NCOM,BLCK ));
		//update zay pengambilan customerlist ke callcategoryID 23-01-2018
		$this->EUI_Page->_setWhereNotIn("a.DM_LastReasonId", array(22, 44));
		//add date expired
...
// atau approve
	...
	if( $cok->cookie(array(USER_ROOT)) ){
		$this->EUI_Page->_setWhereNotIn("a.DM_LastCategoryId", array(APRV,CLOS,BLCK ));
		// $this->EUI_Page->_setWhereNotIn("a.DM_LastCategoryId", array(APRV,CLOS,YCOM,NCOM,BLCK ));
		$this->EUI_Page->_setWhereIn("b.AssignAdmin", $cnf->field('default_admin'));
	}		
...
//edit hilman andi 05112020
	if( $cok->cookie(array(USER_AGENT_OUTBOUND, USER_AGENT_INBOUND))){ 
		$this->EUI_Page->_setWhereNotIn("a.DM_LastCategoryId", array(4,APRV,CLOS,BLCK,13,14));
		// $this->EUI_Page->_setWhereNotIn("a.DM_LastCategoryId", array(NOCT, DECL, APRV,CLOS,YCOM,NCOM,BLCK ));
		// $this->EUI_Page->_setWhereIn("a.DM_CallCategoryId", array(NSTS, FOLW, RDPC));
		$this->EUI_Page->_setWhereNotIn("a.DM_LastReasonId", array(22, 44));
		//add date expired
...
function _select_row_limit_data () {
	...
	$this->db->join("t_gn_campaign c "," a.DM_CampaignId =c.CampaignId", "LEFT");
	#$this->db->where("date(a.DM_DateExpired) >= curdate()");
	$this->db->where("date(c.CampaignEndDate) >= curdate()");
	$this->db->limit(1);
	...
	// if( $this->dataCOK->find_value('UserId') ){
		$this->db->where_in( 'a.DM_LastCategoryId', array(1,2,13));	
	...
	// updated by luqman 05 april 2021
	if ($this->EUI_Page->_getCache("MSD_call_category")) {
		$this->db->where("a.DM_CallCategoryId", $this->EUI_Page->_getCache("MSD_call_category"));	
	}
	if ($this->EUI_Page->_getCache("MSD_last_category")) {
		$this->db->where("a.DM_LastCategoryId", $this->EUI_Page->_getCache("MSD_last_category"));	
	}
	if ($this->EUI_Page->_getCache("MSD_user_agent")) {
		$this->db->where("a.DM_SellerId", $this->EUI_Page->_getCache("MSD_user_agent"));	
	}
	if ($this->EUI_Page->_getCache("MSD_call_start_date")) {
		$this->db->where("a.DM_UpdatedTs >= '".$this->EUI_Page->_getCache("MSD_call_start_date")."'");	
	}
	if ($this->EUI_Page->_getCache("MSD_call_end_date")) {
		$this->db->where("a.DM_UpdatedTs <= '".$this->EUI_Page->_getCache("MSD_call_end_date")."'");	
	}
	if ($this->EUI_Page->_getCache("MSD1_filter_field")) {
		$this->db->like($this->EUI_Page->_getCache("MSD1_filter_field"), $this->EUI_Page->_getCache("MSD1_filter_value"));	
	}
	if ($this->EUI_Page->_getCache("MSD2_filter_field")) {
		$this->db->like($this->EUI_Page->_getCache("MSD2_filter_field"), $this->EUI_Page->_getCache("MSD2_filter_value"));		
	}
...
// untuk menghemat bandwith process Query ke detail master akan di 
// pecah :
 $resultArray = array();
 if(function_exists('MasterIterator') 
 and ($row = MasterIterator( $MasterDataId ) )){
	 
	// push data to here[resultArray]  
	$resultArray['DM_Id'] 					= $row->DM_Id();
	$resultArray['DM_Recsource'] 			= $row->DM_Recsource(); 
	$resultArray['DM_CampaignId'] 			= $row->DM_CampaignId();
	$resultArray['DM_Custno'] 				= $row->DM_Custno();
	$resultArray['DM_FirstName'] 			= $row->DM_FirstName();
	$resultArray['DM_LastName'] 			= $row->DM_LastName(); 
	$resultArray['DM_MotherName'] 			= $row->DM_MotherName();
	$resultArray['DM_Dob'] 					= $row->DM_Dob();
	$resultArray['DM_Age'] 					= $row->DM_Age();
	$resultArray['DM_AddressLine1'] 		= $row->DM_AddressLine1();
	$resultArray['DM_AddressLine2'] 		= $row->DM_AddressLine2();
	$resultArray['DM_AddressLine3'] 		= $row->DM_AddressLine3();
	$resultArray['DM_HomePhoneNum'] 		= $row->DM_HomePhoneNum();
	$resultArray['DM_CallCategoryId'] 		= $row->DM_CallCategoryId(); 
	$resultArray['DM_CallReasonId'] 		= $row->DM_CallReasonId();
	$resultArray['DM_CallReasonKode'] 		= $row->DM_CallReasonKode();
	$resultArray['DM_CallCategoryKode'] 	= $row->DM_CallCategoryKode();
	$resultArray['DM_QualityCategoryId']	= $row->DM_QualityCategoryId();
	$resultArray['DM_QualityReasonId'] 		= $row->DM_QualityReasonId();
	$resultArray['DM_QualityReasonKode']	= $row->DM_QualityReasonKode();
	$resultArray['DM_QualityCategoryKode'] 	= $row->DM_QualityCategoryKode();
	$resultArray['DM_AdmCategoryId'] 		= $row->DM_AdmCategoryId();
	$resultArray['DM_AdmReasonId'] 			= $row->DM_AdmReasonId();
	$resultArray['DM_AdmReasonKode'] 		= $row->DM_AdmReasonKode();
	$resultArray['DM_AdmCategoryKode'] 		= $row->DM_AdmCategoryKode();
	$resultArray['DM_MobilePhoneNum'] 		= $row->DM_MobilePhoneNum();
	$resultArray['DM_ProductId'] 			= $row->DM_ProductId();
	$resultArray['DM_CampaignName'] 		= $row->DM_CampaignName();
	$resultArray['DM_Note'] 	        	= $row->DM_Note();
	 }
	
// return data on array process ;
