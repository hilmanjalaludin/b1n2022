...
{
/**
  * [_getProductCampaignId description]
...
/**
  ...
  function _getProductTypeName()
 {
	...
	foreach( $this ->db -> get() -> result_assoc() as $rows ) {
		$_conds[$rows['ProductType']] = $rows['ProductType'];
	}
...
/**
 ...
 function _getProductCampaignId( $CampaignId=null )  
 {
	...
	// reset select untuk process awal 
	$arr_val = array();
	...
	// get recsource data OK 
	$qry = $this->db->get(); 
	if( $qry && $qry->num_rows() >0 ) foreach( $qry->result_assoc() as $rows ) {
		$arr_val[$rows['ProductId']] = $rows['ProductName'];
	}
...
function _getProductId()
{
 ...
 $sql = " SELECT a.ProductId, a.ProductName, a.ProductCode 
			FROM t_gn_product_master a WHERE a.ProductStatusFlag=1 ";
...
function _getOutboundProductId()
{
	...
	$this -> db ->from("t_gn_campaignproduct a");
	$this -> db ->join("t_gn_product_master b", "a.ProductId=b.ProductId","INNER");
 ...
 */
 ...
 {
	$sql = " SELECT a.PremiumGroupId, a.PremiumGroupDesc 
 ...
 */
 function _get_exist_plan( $Where = array() )
 {
	...
	{	
		...
		$this -> db -> from("t_gn_product_plan");
 ...
 */
 ...
 function _select_product_type( $ProductId = 0 ) 
{
	$rs = $this->db->query("select a.ProductTypeId from t_gn_product a where a.ProductId = '$ProductId'");
	if( ( $rs->num_rows() > 0) 
		AND ( $row = $rs->result_first_assoc()) ) {
		return (int)$row['ProductTypeId'];
	}
	return 0;
}
