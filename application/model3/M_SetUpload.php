...
//@ create object 
 ...
 $this->EUI_Page->_setArraySelect(array(
	...
	"IF(a.TemplateFlags=1, 'Active','Not Active') as TemplateFlags" => array("TemplateFlags", "Status")
 ));
 ...
 */
 ...
 function _submit_row_template( $out = null )  {

// check apakah data yang di kirim object 
   $this->TemplateId = 0; 
   if($out->fields('field_num'))
		$this->result_num_data = $out->fields('field_num');
   
   if( !is_object($out) ){
	   return false;
   }
   
   //pastikan data field nya OK 
   if( is_array( $this->result_num_data ) 
	&& count( $this->result_num_data ) == 0 ){
		return false;	
   }
   
 // push additional data field 
	$out->add('TemplateModul',	$out->field('calass_handler'));
	$out->add('TemplateCreateTs',date('Y-m-d H:i:s'));
	$out->add('TemplateFlags',	1);
	
	
	 
// pertama insert ke table master dahulu 
  
   $this->totalDataRow = 0;
 //  release data  
 
   $this->db->reset_write();
   $this->db->set("TemplateTableName", 	$out->field('table_name')   );
   $this->db->set("TemplateName",		$out->field('templ_name')   );
   $this->db->set("TemplateMode", 		$out->field('mode_input')   );
   $this->db->set("TemplateFileType", 	$out->field('file_type')    );
   $this->db->set("TemplateSparator", 	$out->field('delimiter')    );
   $this->db->set("TemplateBlacklist",	$out->field('black_list')   );
   $this->db->set("TemplateLimitDays",	$out->field('expired_days') );
   $this->db->set("TemplateBucket",		$out->field('bucket_data')  );
   $this->db->set("TemplateCreateTs",	$out->field('TemplateCreateTs') );
   $this->db->set("TemplateFlags",		$out->field('TemplateFlags') );
   $this->db->set("TemplateModul",		$out->field('TemplateModul') );
   
   
   
   
   
   // insert data on table here 
   $this->db->insert("t_gn_template");
   if( $this->db->affected_rows() > 0 ){
		$this->TemplateId =(int)$this->db->insert_id();
   }
   
   //jika hasilnya nill return ini.
   if( !$this->TemplateId ){
	   return false;
   }
   
   //masukan nilai detailnya
   if(is_array($this->result_num_data )) 
   foreach( $this->result_num_data  as $key => $num ){
	   
	   // masukan data ke kolom data maping berikut .
	   $this->fieldDataName  = sprintf("field_names_%s", $num );
	   $this->fieldDataLabel = sprintf("field_label_%s", $num );
	   $this->fieldDataOrder = sprintf("field_order_%s", $num );
	   
	  // kemudian push data nya OK  
	   $this->db->reset_write(); // reset cache 
	   $this->db->set("UploadTmpId", 	 $this->TemplateId);
	   $this->db->set("UploadColsName",  $out->field($this->fieldDataName));
	   $this->db->set("UploadColsAlias", $out->field($this->fieldDataLabel));
	   $this->db->set("UploadColsOrder", $out->field($this->fieldDataOrder));
	   
	   //insert data on here table "t_gn_detail_template"
	   if( $this->db->insert( "t_gn_detail_template" ) ) {
			$this->totalDataRow++;
	   }
    }
    // return callback data to response Server .
	return $this->totalDataRow;
} 
 ...
 * @return [type]             [description]
 */
function _delete_row_template( $result_array = null) {

 $this->cond = 0;	
 if( is_array($result_array) ) 
	foreach($result_array as $key => $TemplateId )
 {
		
	// reset_write cache data on row 
	$this->db->reset_write();
	$this->db->where('UploadTmpId', $TemplateId);
	$this->db->delete('t_gn_detail_template');
		
	// reset master data on table  
	$this->db->reset_write();
	$this->db->where('TemplateId',$TemplateId);
	if( $this->db->delete('t_gn_template') ){
		$this->cond++;
	}
  }	
  
  // return data process on here .
	return $this->cond;
