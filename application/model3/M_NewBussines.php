...
/* 
<02-04-2016> -> abie
NOTES : campaign 19,20,21 adalah campaign UAT dan masih digunakan untuk melakukan pengetestan
 */
class M_NewBussines extends EUI_Model
{
	...
	public function getCountByDate ( $startdates = '' ) {
		...
		$this->getcountdata = $this->db->query("
			...
			and b.FlagExport = 0
			and	date_format(a.QA_UpdateTs , '%Y-%m-%d')='$startdates'
		");
	...
	function getPolicy( $extractData = '' )
	{
		$sql = "select
					...
					'' as seperate_policy, 
					'1' as 'status', 
					occ.OccCode as payer_occupationid, 
					a3.PayerPlaceOfBirth as payer_birthplace, 
					'' as payer_religionid, 
					'' as payer_income, 
					'' as payer_position, 
					'' as payer_company, 
					o.full_name as operid, 
					o.code_user as sellerid, 
				...
				#LOOKUP - LOOKUP
				left join t_lk_occupation occ on occ.OccId=a3.PayerOccupationId
				...
				AND date(b.QA_UpdateTs) = '$extractData'
	...
	function getBeneficiery( $extractData = '' )
	{
		...
		$sql = "select
				...
				from t_gn_beneficiary a
				inner join t_gn_customer d on d.CustomerId=a.CustomerId
				inner join t_gn_policyautogen d1 on a.CustomerId=d1.CustomerId and a.PolicyNumber=d1.PolicyNumber
				left join t_gn_insured a1 on a1.InsuredId=a.InsuredId
				...
				AND date(d.QA_UpdateTs) = '$extractData'
				order by a.PolicyNumber
