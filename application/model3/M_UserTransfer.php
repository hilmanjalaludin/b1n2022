...
/**
  ...
  */
class M_UserTransfer extends EUI_Model
{
...
// tambahakn ke field object 
 ...
 // manager -------------------------------------
 /*
 $objUser = Singgleton('M_SysUser');
 $outUser = Objective( $objUser->_getUserDetail( $UserId ) );
	...
	foreach( $arr_assign as $k => $rows ) 
 {
	...
	$cond = $this->_event_loger_distribute($row->get_value('AssignId'),  $this->callAsgMode);
	if( $cond )
	{
		$this->db->reset_write();
	...
	foreach( $arr_assign as $k => $rows ) 
 {
	...
	$cond = $this->_event_loger_distribute($row->get_value('AssignId'), 'MOV');
	if( $cond )
	{
		$this->db->reset_write();
		$this->db->where('AssignId', (int)$row->get_value('AssignId'));
...
/**
  ...
  * @return [type]             [description]
  */
 function _set_row_transfer_rata( $out = null )  {
	 
// call object --
 $objAsg = Singgleton('M_MgtAssignment');
 $rowAsg = array();
...
// --------- next process ---------------------------------------
 ...
 // def data posted by user ---------------------------
  ...
  $arr_user_avail =& $out->get_array_value('trans_to_user_list');
...
/**
 ...
 public function _set_row_transfer_agent( $out = null  ) 
{
// call object --
