<?php
	...
	class U_data_auto_bagi extends EUI_Upload
	{
		 ...
		 var $_total_column = 0;
		 var $_tbl_assignment = "t_gn_assignment";
		 var $_tbl_master = "t_gn_customer_master";
	...
	*/	
	...
	public function _set_recsource( $recsource = null ) 
	{
 	...
 	{ 
		$arr_template = array();
		$sql = sprintf("select * from t_gn_template a 
			where a.TemplateTableName = '%s'", 
	...
	*/
	 ...
	 * @return [array] 
	 */
 	function _get_data_assign( $id = null)
 	{
 		$this->db->select("*");
	   	$this->db->from($this->_tbl_assignment);
	 ...
	 * @return [array] 
	 */	
 	function _get_seller_kode( $code = null)
 	{
 		$this->db->select("*");
	   	$this->db->from($this->_tbl_master);
	   	$this->db->where("DM_Id", $code);
	   	$_data = $this->db->get();
	...
	{
		// this will call data 	
	   	$data = $this->_get_data_assign( $AssignDataID );
	   	$kode = $this->_get_seller_kode( $data['AssignCustId']);

		// update data di customer untuk dapat data baru 
	   	$val =& CK();
	   	$val->add('DM_Id', $data['AssignCustId']);
	   	$val->add('DM_SellerId', $data['AssignSelerId']);
	   	$val->add('DM_SellerKode', $kode['DM_SellerKode']);

	   	// update table customer master untuk mendapatkan sellerid dan sellerkode
	   	$sql = sprintf("
	   		UPDATE t_gn_customer_master a 
	   		SET a.DM_SellerId ='%s', a.DM_SellerKode ='%s' 
	   		WHERE a.DM_Id= %d ", 
			    $val->field('DM_SellerId', array('SetCapital')),
				$val->field('DM_SellerKode', array('SetCapital')),
	 ...
	 */	
 	function _set_write_verification( $disposition = 0, $row = null )
 	{
		...
		if( $disposition ) {
			$sql = sprintf("
				update t_gn_customer_verification a 
				inner join t_gn_customer_master b on a.CV_Data_Custno=b.DM_Custno
	 ...
	 * @return [type]             [description]
	 */		
	protected function _set_assignment( $ID = 0 , $AgentCode=null )
	{

		//GET aggent by user kode untuk mendapatkan
		//assign_spv, assign_mgr, assign_seller_id
		$Agent  =& $this->M_SysUser->_getUserByCode($AgentCode);
		
		// ambil session data dari modul cok 
		$cok = CK();
		$cok->add('AssignCustId', $ID);
		$cok->add('AssignDate', $this->_upload_data_date);
		$cok->add('AssignMode', 'DIS');
		...
		$this->db->reset_write();
	 	...
	 	if( !strcmp( $cok->field('HandlingType'), USER_ROOT )){
			...
			$this->db->set('AssignSpv', $Agent['spv_id']);
			$this->db->set('AssignMgr', $Agent['mgr_id']);
			$this->CondID++;
		}
		 // ambil hiraki 
		...
		// then next process 
	  	...
	  	if(is_array($this->_upload_table_data)) {
			foreach( $this->_upload_table_data as $n => $values )
			{
				...
				$val->add('DM_SellerKode',		$val->field('DM_SellerKode', array('SetStringVal')));
				...
				if( is_array( $result_array ) ) {
					...
					// data OK 
					$callDispositionID = 0;
					// print_r($this->_upload_table_name);exit();
					...
					// echo $this->_writeAssignDataId;
					// insert ke table assigment dengan data balikan dari customer 
					$this->_writeAssignDataId = $this->_set_assignment( $callDispositionID ,$val->field('DM_SellerKode', array('SetStringVal')));
	...
	private function _get_data_customer_last()
	{
		$sql = "
			SELECT tgm.* ,ta.* 
			FROM t_gn_customer_master tgm 
			INNER JOIN tms_agent ta ON ta.UserId = tgm.DM_SellerId
			WHERE tgm.DM_UploadedTs= '".$this->_upload_data_date."'
			";
	...
	// function _uptodate()
	// {
	// 	$data = $this->_get_data_customer_last();

	// 	foreach( $data as $row )
	// 	{
	// 		$sql = "
	// 				UPDATE t_gn_assignment ta 
	// 				SET ta.AssignSpv = '$row['spv_id']'
	// 				WHERE ta.AssignCustId IN ($row['DM_Id'])
	// 			";
	// 		echo $sql;
	// 		return $this->db->query($sql);
	// 	}
