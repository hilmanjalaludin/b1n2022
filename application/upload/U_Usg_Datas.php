...
class U_Usg_Datas extends EUI_Upload
{
...
private function _set_write_assignlogasli( $AssignDataID  = 0, $agent=null ){
// var_dump($agent);die;
...
// update data di customer untuk dapat data baru 
   ...
   $sqlx = sprintf("UPDATE t_gn_customer_master a SET a.DM_SellerId ='%s', a.DM_SellerKode ='%s' WHERE a.DM_Id= %d ", 
...
// $this->db->set('AssignSelerId', $agent);
 ...
 function _set_write_verification( $disposition = 0, $row = null ){
  ...
  // konvert ke bentuk Object OK 
	...
	if( !$disposition ){
		...
		$this->db->set('CV_Data_MemberSince',  $row->field('DM_MemberSince'));
		$this->db->set('CV_Data_Usia',  $row->field('DM_Usia'));
 ...
 * @return [type]             [description]
 */		
protected function _set_assignment( $DebiturId = 0 , $AgentCode=null, $agent=null )
{
...
//  $this->db->reset_write();
 ...
 if( !strcmp( $cok->field('HandlingType'), USER_ROOT )){
	...
	// $this->db->set('AssignSelerId', $cari['UserId']);
	$this->CondID++;
 }
 // ambil hiraki 
...
protected function _set_write_assignlog( $DebiturId = 0 , $AgentCode=null, $agent=null )
{
...
// then next process 
  ...
  {
	$query="SELECT a.UserId FROM tms_agent a WHERE a.id in( '".$values['Agent']."')";
	$cari = $this->db->query($query)->row_array();
	// $this->_set_write_assignlog( $callDispositionID,null, $cari['UserId'] );die;
	unset($values['Agent']);
...
// 	@call_user_func_array untuk data verifikasi .
	@call_user_func_array( array($this,'_set_write_verification'), 
...
// hingga bersih.
	...
	// edit hilman di matiin dulu buat cek assigmnment
	...
	//  var_dump($callDispositionID);
	//  var_dump($val->field('Agent'));
	
	 $this->_writeAssignDataId = $this->_set_assignment( $callDispositionID, null, $cari['UserId'] );
	//  echo "kuy";
	// die;

	if( $this->_writeAssignDataId ){
		$this->_set_write_assignlog( $callDispositionID,null, $cari['UserId'] );
		// $this->_set_write_assignlog( $callDispositionID, null, $cari['UserId'];
	}
