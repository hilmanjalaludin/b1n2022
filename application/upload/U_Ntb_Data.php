...
class U_Ntb_Data extends EUI_Upload
{
...
// query syntak insert to loger data process .
	...
	/* -- untuk tutup process assigment log ok */
	/*
	if( !$this->db->query($sql) ){
...
// then next process 
 ...
 // master customer .
	...
	$val->add('DM_OfficePhoneNum', 	$val->field('DM_OfficePhoneNum', array('SetStringVal')));
	$val->add('DM_MobilePhoneNum', 	$val->field('DM_MobilePhoneNum', array('SetStringVal')));
	$val->add('DM_OtherPhoneNum',	$val->field('DM_OtherPhoneNum', array('SetStringVal')));
 ...
 // berikut ini.
	...
	// push data to DB setelah data di cuci hingga bersih.
	...
	if( $this->db->affected_rows()>0 ) {
		...
		// ambil data customer ID / last insert_id  untuk dimasukan ke table2 terkait.
	...
	// if error inserting data .
	else{
		$ths->_mysql_error[] = mysql_error();
		// duplicate error<1062> kode lihat di ref mysql error 
		// https://dev.mysql.com/doc/refman/5.7/en/error-messages-server.html
		
		if( !strcmp($this->db->_error_number(), 1062 ) ){
