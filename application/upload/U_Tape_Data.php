...
class U_Tape_Data extends EUI_Upload
{
...
// update data di customer untuk dapat data baru 
   ...
   $sql = sprintf("UPDATE t_gn_customer_master a SET a.DM_SellerId ='%s', a.DM_SellerKode ='%s' WHERE a.DM_Id= %d ", 
 ...
 function _set_write_verification( $disposition = 0, $row = null ){
  ...
  // konvert ke bentuk Object OK 
	...
	if( !$disposition ){
		...
		$this->db->set('TapenasCIF',  			$row->field('DM_Custno') );
		$this->db->set('TapenasKodeCabang',		$row->field('DM_KodeCabang') );
		$this->db->set('TapenasNoRekening',		$row->field('DM_NoRekening') );
		$this->db->set('TapenasKodeWilayah',	$row->field('DM_Wilayah') );
		$this->db->set('TapenasPenerimaBenef',	$row->field('DM_PenerimaManfaat') );
		$this->db->set('TapenasTglBuka',		$row->field('DM_TglBuka') );
		$this->db->set('TapenasTglJatuhTempo',	$row->field('DM_TglJatuhTempo') );
		$this->db->set('TapenasJangkaWaktu',	$row->field('DM_Tenor') );
		$this->db->set('TapenasSetoran',		$row->field('DM_SetorBulanan') );
		$this->db->set('TapenasRekAfilieasi',	$row->field('DM_RekAfiliasi') );
		$this->db->set('TapenasAlamat',			$row->field('DM_AddressLine1') );
		
		// jika process Berhasil SIP Ya .
		if( $this->db->insert('t_gn_customer_tapenas') ){
			$num++;
		}
	}
	...
	if( $disposition ){
		$sql = sprintf("update t_gn_customer_tapenas a 
						inner join t_gn_customer_master b on a.TapenasCIF=b.DM_Custno
						set a.TapensasCustId=b.DM_Id
...
// then next process 
 ...
 // master customer .
	...
	$val->add('DM_TglBuka', 		$val->field('DM_TglBuka', array('SetDateEnglish')));
	$val->add('DM_TglJatuhTempo', 	$val->field('DM_TglJatuhTempo', array('SetDateEnglish')));
