...
class U_Usg_Data extends EUI_Upload
{
 ...
 */	
 ...
 function _set_write_verification( $disposition = 0, $row = null ){
	...
	$row['DM_Usia'] = $row['DM_Tenor'];
	unset($row['DM_Tenor']);
	$row['DM_MemberSince'] = $row['DM_SetorBulanan'];
	unset($row['DM_SetorBulanan']);
  ...
  // konvert ke bentuk Object OK 
	...
	if( !$disposition ){
		...
		$this->db->set('CV_PerisaiPlus', ($row->field('DM_Note') == 'Y' || $row->field('DM_Note') == 'T'?$row->field('DM_Note'):NULL));
...
// then next process 
  ...
  {
	  $values['DM_Tenor'] = $values['DM_Usia'];
	  unset($values['DM_Usia']);
	  $values['DM_SetorBulanan'] = $values['DM_MemberSince'];
	  unset($values['DM_MemberSince']);
	//   print_r($values);
	//   die;
 ...
 // master customer .
	...
	$val->add('DM_Note', 		$val->field('DM_Note', array('SetStringVal', 'SetCapital')));
