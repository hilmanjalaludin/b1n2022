	...
	public function ShowExcel () {
		...
		if ( $gao->get_value("mode") == "EXCEL" ) {
			...
			switch ( $productid ) {
				...
				// ADD PRODUCT ID < t_gn_product_master>
				case ADD :
					$xcelFname = "ReportProductivity_".$this->product[$productid]."_".date("ddmmyy");
					$dao = array(
								// "param" => $gao , 
				...
				// USAGE PRODUCT ID < t_gn_product_master>
				case USAGE :
					$xcelFname = "ReportProductivity_".$this->product[$productid]."_".date("ddmmyy");
					$dao = array(
								// "param" => $gao , 
...
count(distinct(if(mst.DM_QualityReasonKode = 'CLOS' and mst.DM_CallDateTs >= '".$StartDate."' and mst.DM_CallDateTs <= '".$EndDate."' ,1,NULL))) as Closedeal,
			...
			/*if($Supervisor){
					$query_ntb .=" And agentSpv.spv_id = $Supervisor ";
				}
				if($Tmr){
	...
	// if($Supervisor){
		// $query_ntb .=" And tms.spv_id = $Supervisor ";
	// }
	// if($Tmr){
...
select
					...
					his.TotalClos
				from t_gn_callhistory a USE INDEX(CallHistoryCreatedTs)
					...
					left join (select
							...
							from t_gn_callhistory ch USE INDEX(CallHistoryCreatedTs)
								inner join tms_agent ag on ch.CreatedById = ag.UserId
							where ch.CallReasonId = 22 and ag.handling_type not in (19,22)
								and ch.CallHistoryCreatedTs >= '2019-07-24 00:00:00'  and ch.CallHistoryCreatedTs <='2019-07-24 23:59:58'
							group by ch.CreatedById) his on a.CreatedById = his.CreatedById and his.Tgl = d.DM_CallDateTs
					left join tms_agent e on b.spv_id = e.UserId
				where a.CallHistoryCreatedTs >= '2019-07-24 00:00:00' 
					AND a.CallHistoryCreatedTs <= '2019-07-24 23:59:58'  
					and b.spv_id = 62  
					and a.CreatedById in (71,76,89,91,95,158,159,197,316,426,427,435,448,468,474,490,498,502,558,561,568)  
					and d.DM_CampaignId in (29)  group by a.CreatedById
					*/
	...
	$query_ntb = "select
				...
				from t_gn_callhistory a USE INDEX(CallHistoryCreatedTs)
					...
					left join (select
							...
							group by Tgl, ch.CreatedById) his on a.CreatedById = his.CreatedById and his.Tgl = d.DM_CallDateTs
				where  a.CallHistoryCreatedTs >=  '".$StartDate."' and a.CallHistoryCreatedTs <= '".$EndDate."' ";
	if($Supervisor){
		$query_ntb .=" and b.spv_id = $Supervisor ";
	}
	if($Tmr){
    ...
    public function _get_data_usage(){
		...
		if ( !empty($CampaignId) AND !empty($StartDate) AND !empty($EndDate) ) {
			...
			$query_usg     = " SELECT a.*, b.DM_Recsource, g.PRD_Data_Value 
					...
					if($Supervisor){
						$query_usg .=" And f.AssignSpv = $Supervisor ";
					}
					if($Transaksi){
