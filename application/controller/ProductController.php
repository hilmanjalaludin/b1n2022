...
class ProductController extends EUI_Controller
{
	...
	public function DeleteTransaction()
	{
		...
		$deleting = $this->{base_class_model($this)}->_delete_transaction_usage($gd->get_value('SequenceNo'), $gd->get_value('TransacId'), $gd->get_value('VerifycaId'), $gd->get_value('custID'));
	...
	// add dida
	public function saveStatusUsage() {
		$save = $this->{base_class_model($this)}->_save_status_usage($this -> URI);
		if($save) {
	...
	// 2021-11-26
	public function getPerisaiPlusStatus(){
		$status = $this->{base_class_model($this)}->_getPerisaiPlusStatus($this -> URI);
		// if($status) {
			// echo json_encode(array('status' => 1));
		// } else {
	...
	public function checkFormUsage(){
		$jumlah = $this->{base_class_model($this)}->_checkFormUsage($this -> URI);
		// if($jumlah) {
			// echo json_encode(array('jumlah' => 1));
		// } else {
			// echo json_encode(array('jumlah' => 0));
		// }
		// print_r($jumlah);
		echo json_encode($jumlah);
	}
	
	public function updatePerisaiPlus(){
		$success = $this->{base_class_model($this)}->_updatePerisaiPlus($this -> URI);
		if($success) {
