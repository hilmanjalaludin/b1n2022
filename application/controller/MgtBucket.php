...
function exportFixid(){
	// php /opt/enigma/webapps/bnitele_2022/index.php MgtBucket exportFixid
	$this->load->upload('U_Refresh_DataVerification');
	$Uploader = call_user_func(array('U_Refresh_DataVerification', "Instance"));
	if(class_exists('U_Refresh_DataVerification')){
		$Uploader->_exportFixid();
		echo 'done';
	}else{
		echo 'notfound';
	}
...
function downloadFixid(){
	$this->load->upload('U_Refresh_DataVerification');
	$Uploader = call_user_func(array('U_Refresh_DataVerification', "Instance"));
	if(class_exists('U_Refresh_DataVerification')){
		$filepathname = $Uploader->_exportFixid(1);
...
function UploadBucketchminat() {
  ...
  // for properies .
   ...
   // konversi data ke bentuk asli process 
	...
	and isset($this->ar_val_file['fileTouploadx'] ) ){
		
	   foreach( $this->ar_val_file['fileTouploadx'] as $key => $val ){
...
//    die;
  ...
  // ubah variable array ke bentuk object 	untuk 
  // di process lebih lanjut.
	 $this->stm = $this->fls->field('fileTouploadx', 'Objective');
...
//    die; 
			...
			!strcmp($this->tpd->field('TemplateBucket'), 'Y') ) )  
  {
	  ...
	  if(!strcmp( $this->tpm, 'INSERT')) {
		$this->callbackUpl = $this->upl->setInsertUploads( 
							 array( 'file_attribut' => $this->ar_val_file, 'request_attribut' => $this->param ), 
							 array( 'Upload_DateTs' => date('Y-m-d H:i:s'), 'Upload_ById'     => $this->cok->field('UserId')));
...
function UpdateCache(){
 ...
 function UpdateData() {
	...
	if( is_array($this->ar_val_file) and isset($this->ar_val_file['fileTorefresh'] ) ){
