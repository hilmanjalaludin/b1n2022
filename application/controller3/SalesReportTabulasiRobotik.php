...
class SalesReportTabulasiRobotik extends EUI_Controller
{
	...
	public function __construct(){
		...
		$this->folder = "rpt_sales_report_tabulasi_robotik/";
		$this->folderCampaign = "rpt_sales_report_tabulasi_robotik/rpt_per_campaign/";
	}
	...
	public function index(){
		...
		$out = &get_class_instance('M_SalesReportTabulasiRobotik');
		$out_data = array(
			"Campaign" => array(),
			"Supervisor" => $this->M_SalesReportTabulasiRobotik->supervisor(),
			"Transaksi" => $this->M_SalesReportTabulasiRobotik->transaksi()
		);
		$this->load->view("rpt_sales_report_tabulasi_robotik/report_sales_nav", $out_data);
	}
		...
		if ($this->EUI_Session->_get_session('UserId') && $this->EUI_Session->_get_session('HandlingType') != '4') {
			...
			if ($gao->get_value("mode") == "HTML") {
				...
				switch ($productid) {
					...
					case 22:
						$this->load->view(
							$this->folderCampaign . $mode_report . "/rpt_" . $this->product[$productid] . "_html",
							array(
								"param" => $gao
								/*
	...
	function ShowCampaignByProductId(){
		...
		echo form()->combo(
			'campaign_id',
			'select auto x-select',
			$result_array,
			null,
			array("change" => "window.showRecsourceByCampaignId(this);")
		);
			...
			switch ($productid) {
				...
				case USAGE:
					$result_array = array(
						...
						"datesend" => $EndDate,
						"merchants" => $this->getMerchantName(USAGE),
						"merchantsTenor" => $this->getMerchantsTenor(USAGE)
					);
					break;
				case 27:
					$result_array = array(
	...
	public function ShowExcel(){
		...
		if ($this->EUI_Session->_get_session('UserId') && $this->EUI_Session->_get_session('HandlingType') != '4') {
			...
			$productid = $gao->get_value("product_id");//$this->newproduct[$campaignId];
			// echo $productid; die();
			// print_r($this->product[$productid]);
			// $xcelFname = "report";
			// switch ( $campaignId ) {
				...
				switch ($productid) {
					case 22:
						...
						$dao = array(
							...
							"data_ntb_dualcard" => $this->_get_data_dualcard_ntb(),
							"data_addon_ntb" => $this->_get_data_addon_ntb(),
							"data_addon_ntb_1" => $this->M_SalesReportTabulasiRobotik->_get_data_addon_ntb_1(),
							"data_addon_ntb_2" => $this->M_SalesReportTabulasiRobotik->_get_data_addon_ntb_2()
						);
					...
					case 27:
						$xcelFname = "Tabulasi_" . $this->newcampaign[$campaignId] . "_" . date("ddmmyy");
						$xcelFname = preg_replace('/\s+/', '_', $xcelFname);
						$dao = array(
							// "param" => $gao , 
							// "data_ntb" => $this->_get_data_ntb()
							// "data_ntb_dualcard" => $this->_get_data_dualcard_ntb() , 
							...
							"param" => $gao,
							"data_balcon" => $this->_get_data_balcon()
							//								"data_ntb_dualcard" => $this->_get_data_dualcard_ntb() , 
							//								"data_addon_ntb" => $this->_get_data_addon_ntb()
						);
						$Product = $this->product[$productid];
	...
	public function getMerchantName($param = null){
		$Merchant = array();
		if($param){
			$Merchant = $this->M_SalesReportTabulasiRobotik->_getMerchantName($param);
		}
		
		return $Merchant;
	}public function getMerchantsTenor($param = null){
		$Merchant = array();
		if($param){
			$Merchant = $this->M_SalesReportTabulasiRobotik->_getMerchantsTenor($param);
		}
	...
	public function _get_data_usage(){
		...
		if (!empty($CampaignId) and !empty($StartDate) and !empty($EndDate)) {
			...
			if ($Transaksi == 1) {
			...
			else if ($Transaksi == 'MANUAL') {
				$query_usg     = "SELECT a.*, b.DM_Recsource , g.PRD_Data_Value , b.DM_spvid, 
						b.DM_QualityUserId, h.code_user, b.DM_MobilePhoneNum FROM t_gn_frm_usage a
						INNER JOIN (SELECT suba.TX_Usg_FixID,
							if(suba.TX_Usg_ProgramValue = 'XD 0%', if(suba.TX_Usg_Tenor=3, suba.TX_Usg_Custno, 0), suba.TX_Usg_Custno) AS TX_Usg_Custno,
							if(count(suba.TX_Usg_TransSeq) > 1, if(max(suba.TX_Usg_TransSeq)>1, MAX(suba.TX_Usg_TransSeq),0), 0) as MaxSeq
							FROM t_gn_frm_usage suba
							INNER JOIN t_gn_customer_master sub ON suba.TX_Usg_CustId = sub.DM_Id
							WHERE sub.DM_QualityUpdateTs >= '$StartDate' AND sub.DM_QualityUpdateTs <= '$EndDate'
							and suba.TX_Usg_ProgramValue in ('XD 0,75%','XD 0%')
							GROUP BY TX_Usg_Custno) Seq on a.TX_Usg_Custno = Seq.TX_Usg_Custno
						AND a.TX_Usg_TransSeq = Seq.MaxSeq
						LEFT JOIN t_gn_customer_master b ON a.TX_Usg_CustId = b.DM_Id
						INNER JOIN t_gn_assignment f on b.DM_Id = f.AssignCustId
						INNER JOIN t_lk_program_detail g on a.TX_Usg_ProgramData=g.PRD_Data_Id
						...
						WHERE 1=1
						and b.DM_QualityReasonId = 44
						and a.TX_Usg_ProgramValue in ('XD 0,75%','XD 0%') /*,'XD 0,65%'*/
						and b.DM_QualityUpdateTs >= '$StartDate' AND b.DM_QualityUpdateTs <= '$EndDate'";
				if ($Supervisor) {
					$query_usg .= " And f.AssignSpv in ($Supervisor) ";
				}
			...
			else if ($Transaksi == 'ROBOTIK') {
				$query_usg_gabungan1     = "SELECT * FROM (SELECT a.*, b.DM_Recsource , g.PRD_Data_Value , b.DM_spvid, 
					...
					INNER JOIN
						(SELECT TX_Usg_Custno,
						 ...
						 if(TX_Usg_ProgramValue<>'XD 0,75%' AND count(TX_Usg_TransSeq) = 1, TX_Usg_TransSeq,0)) AS MaxSeq
						 FROM t_gn_frm_usage a LEFT JOIN t_gn_customer_master b ON a.TX_Usg_CustId = b.DM_Id
						 WHERE b.DM_QualityUpdateTs >= '$StartDate' AND b.DM_QualityUpdateTs <= '$EndDate'
							and TX_Usg_ProgramValue<>'XD 0,75%' GROUP BY TX_Usg_Custno) Seq
					...
					AND g.PRD_Data_Value != 'SS 1%' 
					AND g.PRD_Data_Value != 'SS 1.25%'
					AND (TX_Usg_ProgramValue ='XD 0%' AND TX_Usg_TransSeq < 2)
				...
				$query_usg_gabungan2     = "select * from (SELECT a.*, b.DM_Recsource , g.PRD_Data_Value , b.DM_spvid, 
					b.DM_QualityUserId, h.code_user, b.DM_MobilePhoneNum, CONCAT(TX_Usg_ProgramValue,'-',2) AS MaxSeq FROM t_gn_frm_usage a
					INNER JOIN (SELECT
						if(a.TX_Usg_ProgramValue = 'XD 0%', if(a.TX_Usg_Tenor<>3, a.TX_Usg_Custno, 0), a.TX_Usg_Custno) AS TX_Usg_Custno,
						if(TX_Usg_ProgramValue not in ('XD 0,75%') AND count(TX_Usg_TransSeq) > 1, max(TX_Usg_TransSeq), 0) AS MaxSeq
						FROM t_gn_frm_usage a LEFT JOIN t_gn_customer_master b ON a.TX_Usg_CustId = b.DM_Id
						WHERE b.DM_QualityUpdateTs >= '$StartDate' AND b.DM_QualityUpdateTs <= '$EndDate' GROUP BY TX_Usg_FixID) Seq
						ON a.TX_Usg_Custno = Seq.TX_Usg_Custno AND a.TX_Usg_TransSeq = Seq.MaxSeq
					LEFT JOIN t_gn_customer_master b ON a.TX_Usg_CustId = b.DM_Id
					INNER JOIN t_gn_assignment f on b.DM_Id = f.AssignCustId
					INNER JOIN t_lk_program_detail g on a.TX_Usg_ProgramData=g.PRD_Data_Id
					LEFT JOIN tms_agent h on b.DM_QualityUserId = h.UserId
					...
					AND g.PRD_Data_Value != 'SS 1.25%'*/
					/*and a.TX_Usg_ProgramValue <> 'XD 0,75%'
					AND g.PRD_Data_Value != 'XD 0%'*/
					and b.DM_QualityUpdateTs >= '$StartDate' AND b.DM_QualityUpdateTs <= '$EndDate') as Zed WHERE (Zed.MaxSeq <> 'XD 0,75%-2')";
				...
				$query_usg_gabungan3     = "SELECT a.*, b.DM_Recsource , g.PRD_Data_Value , b.DM_spvid, 
					b.DM_QualityUserId, h.code_user, b.DM_MobilePhoneNum, CONCAT(TX_Usg_ProgramValue,'-',1) AS MaxSeq FROM t_gn_frm_usage a
					INNER JOIN
						(SELECT TX_Usg_Custno, /*TX_Usg_ProgramValue in ('XD 0,75%','XD 0,925%')*/
						 /* if(COUNT(TX_Usg_id)>1,MIN(TX_Usg_TransSeq),0) AS MaxSeq */
						 if(COUNT(TX_Usg_TransSeq)>1,MIN(TX_Usg_TransSeq),if(COUNT(TX_Usg_TransSeq)=1,MIN(TX_Usg_TransSeq),0)) AS MaxSeq
						 FROM t_gn_frm_usage a LEFT JOIN t_gn_customer_master b ON a.TX_Usg_CustId = b.DM_Id
						 WHERE b.DM_QualityUpdateTs >= '$StartDate' AND b.DM_QualityUpdateTs <= '$EndDate'
						 GROUP BY TX_Usg_Custno) Seq
							ON a.TX_Usg_Custno = Seq.TX_Usg_Custno AND a.TX_Usg_TransSeq = Seq.MaxSeq
					LEFT JOIN t_gn_customer_master b ON a.TX_Usg_CustId = b.DM_Id
					INNER JOIN t_gn_assignment f on b.DM_Id = f.AssignCustId
					INNER JOIN t_lk_program_detail g on a.TX_Usg_ProgramData=g.PRD_Data_Id
					LEFT JOIN tms_agent h on b.DM_QualityUserId = h.UserId
					WHERE 1=1
					/*and TX_Usg_ProgramValue in ('XD 0,925%')*/
					and b.DM_QualityReasonId = 44
					/*AND g.PRD_Data_Value != 'SS 1%' 
					AND g.PRD_Data_Value != 'SS 1.25%'*/
					AND g.PRD_Data_Value != 'XD 0%'
				...
				$query_usg = $query_usg_gabungan1 . ' UNION ' . $query_usg_gabungan2 . ' UNION ' . $query_usg_gabungan3 . ' ) AS NewTable group by NewTable.TX_Usg_Id';
				// $query_usg =  'select * from ('.$query_usg_gabungan3.') AS NewTable group by NewTable.TX_Usg_Id';
				// var_dump($query_usg_gabungan1);
				// die();
			}
			elseif ($Transaksi == 5) {
				$query_usg     = "SELECT a.*, b.DM_Recsource , g.PRD_Data_Value , b.DM_spvid, 
				...
				$query_usg     = "SELECT a.*, b.DM_Recsource , g.PRD_Data_Value , b.DM_spvid, 
						b.DM_QualityUserId, h.code_user, b.DM_MobilePhoneNum,
						CONCAT(a.TX_Usg_ProgramValue,'-',a.TX_Usg_TransSeq) AS ValueConcat
	...
	public function _get_data_balcon()
	{
		...
		if (!empty($CampaignId) and !empty($StartDate) and !empty($EndDate)) {
			...
			$query_usg     = "SELECT a.*, b.DM_Recsource , g.PRD_Data_Value , b.DM_spvid, 
						...
						CAST(a.TX_Usg_AvailableSS AS SIGNED) AS TxAvailableSS, a.TX_Usg_Tigabulan, concat('0',CAST(a.TX_Usg_AvailableSS AS SIGNED)) as TxAvailableSSD
			...
			$query_usg .= " GROUP BY a.TX_Usg_FixID";
			$query_usg .= " ORDER BY a.TX_Usg_Id";
			// echo "data sini <br>";
			// var_dump($query_usg);
			// die;
	...
	public function _get_data_pctd()
	{
		$data_usage = false;
		$this->data_usage = $data_usage;
		$param_request = _get_all_request();
		...
		if (!empty($CampaignId) and !empty($StartDate) and !empty($EndDate)) {
			...
			$EndDate   = _getDateEnglish($EndDate) . " 23:59:58";
			$query_usg     = "SELECT a.*, b.DM_Recsource , g.PRD_Data_Value , b.DM_spvid,
						b.DM_QualityUserId, h.code_user, b.DM_MobilePhoneNum FROM t_gn_frm_pctd a
						INNER JOIN (SELECT TX_Usg_Custno, if(count(TX_Usg_TransSeq) = 1, TX_Usg_TransSeq, min(TX_Usg_TransSeq)) as MaxSeq
						FROM t_gn_frm_pctd GROUP BY TX_Usg_FixID) Seq on a.TX_Usg_Custno = Seq.TX_Usg_Custno
						AND a.TX_Usg_TransSeq = Seq.MaxSeq
						LEFT JOIN t_gn_customer_master b ON a.TX_Usg_CustId = b.DM_Id
						INNER JOIN t_gn_assignment f on b.DM_Id = f.AssignCustId
						LEFT JOIN t_lk_program_detail g on a.TX_Usg_ProgramData=g.PRD_Data_Id
						LEFT JOIN tms_agent h on b.DM_QualityUserId = h.UserId
						WHERE 1=1
						and b.DM_LastReasonId = 44
						and a.TX_Usg_UpdatedTs >= '$StartDate' AND a.TX_Usg_UpdatedTs <= '$EndDate'";
			if ($Supervisor) {
				$query_usg .= " And f.AssignSpv in ($Supervisor) ";
			}
			if ($Tmr) {
			...
			$query_usg .= " GROUP BY a.TX_Usg_Custno ORDER BY a.TX_Usg_Id";
