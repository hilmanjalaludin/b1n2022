<?php
  ...
  */
class Rekontest extends EUI_Controller{
...
// get my role data process on form action 
 ...
 $this->load->view("mod_user_rekontest/view_rekontest_index", array(
...
// result_array  to process 	
	$result_array = call_user_func('CallResultByCategoryRekontest', $URI->field('callStatus'));
...
// http://192.168.10.236/bni-tele-ans/index.php/Rekontest/CallReasonDataId
/**
 * [Recovery data failed upload HSBC TAMAN SARI]
 * @param  [type] $CustomerId [description]
 * @return [type]             [description]
...
// "result_count"
 ...
 // set data view client porcess  
  $this->load->view("mod_user_rekontest/view_rekontest_row_page", array(
...
// get load model 
 $this->load->model( array('M_UserRekontest') );
 $Rekontest = &Singgleton('M_UserRekontest');
...
// on act quantity  
 ...
 if( $out->field( 'torek_useraction' )){
	$Rekontest->_set_row_rekontest_perrata( $out );
	$totalDataProcess = $Rekontest->_get_call_back_message();
	if( $totalDataProcess ){
