...
{ 
	function _select_report_spv_value()
 {
	...
	$sql[] = sprintf("select  a.UserId, a.id, a.full_name from tms_agent a
					  where a.handling_type = %d", USER_SUPERVISOR);
...
// get result data of manager from DB 	
	$result_supervisor = _select_report_spv_value(); 
...
// switch data to process OK 	
	...
	switch( $URI->field('interval') )
	{
		...
		foreach( $result_supervisor as $SupervisorId => $SupervisorName ){
			...
			call_user_func( '_select_report_process_summary', $SupervisorId );
		}
		break;
 ...
 function _showheaders()
{
	...
	echo "<div class=\"center\">".
		 "<p class=\"normal font-size22\">Report Call Tracking - Summary By Campaign Per SPV</p>".
...
<html>
	<head>
		<title><?php echo title_header("Call Tracking - Summary By Campaign Per SPV");?></title> 
