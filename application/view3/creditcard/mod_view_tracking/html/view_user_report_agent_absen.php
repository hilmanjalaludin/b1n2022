 ...
 function _select_report_agent_absensi()
{
	...
	$arr_var_agents = _select_report_agent_value();
	...
	$sql_str[]= "SELECT 
				...
				CONCAT( DATE_FORMAT(a.ActivityDate, '%Y-%m-%d'),' ',  '08:30:00') as work_log,
	...
	$sql_str[]= sprintf( "WHERE a.ActivityEvent IN('ACTION_EVENT_LOGIN',  'ACTION_EVENT_LOGOUT')
						  AND a.ActivityUserId IN(%s)
						  AND a.ActivityDate >='$start_date 00:00:00' 
						  AND a.ActivityDate <='$end_date 23:59:59'
						  GROUP BY tgl, agent_id ", 
							SetWhereIn( array_keys( $arr_var_agentid), 1));
...
// --- load data will loop -------------------
	...
	while(true)
	{
		...
		print("<table class=\"data\" border=1 style=\"border-collapse: collapse\" width=\"100%\" align=\"center\" >
			<tr>
				<td class=\"head\" align=\"center\">No</td> 
				<td class=\"head\">Agent ID</td> 
				<td class=\"head\">Agent Name</td> 
	...
	// -------- select row ---------------- */
		...
		if( is_array( $arr_var_agents) ) 
			foreach( $arr_var_agents as $AgentId => $AgentName )
		{
		...
		// --- this will default  ---------------
			...
			// --- get leader --------
			$LeaderUsername = $row_spv->field('Username');
			$LeaderFullname = $row_spv->field('full_name');
			
			$no++;
			printf("%s", "<tr>");
				printf("<td class=\"content\" align=\"center\">%s</td>", $no); 
				printf("<td class=\"content\">%s</td>", $row_val->field('id','strtoupper')); 
				printf("<td class=\"content\">%s</td> ", $row_val->field('full_name', 'strtoupper'));
...
<html>
	<head>
		<title><?php echo title_header("User Tracking - Summary Agent Absensi");?></title> 
