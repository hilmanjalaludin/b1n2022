...
// jika process call memang sedang terjadi.	
 ...
 window.EventCallResultID = function( dropdown ) {
 	...
 	var marlin = Ext.Cmp('CallStatusId').getValue();
...
var  frmAdminPaperWorkData = Ext.Serialize('frmAdminPaperWork');
	  frmdminPaperWorkReqs = new Array( 'SV_Cust_MailAddress', 'SV_Cust_FaxNum', 	
										'SV_Cust_Kurir');
		
		
	// default required process 
	  frmAdminPaperWorkData.Complete(frmdminPaperWorkReqs);
  ...
  if( !frmAdminPaperWorkData ){ 
	Ext.Msg('Input form not complete').Info();
	console.log( frmAdminPaperWorkData);
	return false;
 }
...
if( this.Utils.IsObject(callDataConn.event)  
	...
	if( !window.PolicyReady().PolicyReady ){
...
// submit data di process 
 ...
 var frmActivityCall   = Ext.Serialize('frmActivityCall').Initialize(),
	 ...
	 frmAdminPaperWork = Ext.Serialize('frmAdminPaperWork').Initialize(),
	 frmAdditionalCall = {
		CustomerId 		: Ext.Cmp('DM_Id').getValue(),
 ...
 var dataConnUrl = Ext.EventUrl( new Array('ModSaveActivity/SaveAdminActivity'));
	 Ext.Ajax
 ({
	url 	: dataConnUrl.Apply(), 
	method : 'POST',
	param   : Ext.Join( new Array( frmActivityCall,   frmInfoCustomer,  
								   frmAdditionalCall, frmActivityData, frmAdminPaperWork )).object(),
	success  : function( xhr ) {
		 Ext.Util( xhr ).proc(function( dataConnServer ){
			if( dataConnServer.success ) {
				...
				window.PageCallHistory({page : 0, orderby : "", type : ""});				
			} else{
	...
	if( !tabpanel ) {
		...
		$('#tab-bottom-admin-form').Spiner ({
			...
			complete : function( html ){
				// console.log( html );	
				$(html).css({'height' : '100%'});
				$('#form-title-product').html( ProductName );
				
				window.setTimeout( function(){
					$('.wraplable').each( function(){
						$(this).find('select').attr('disabled', true);
						$(this).find('input').attr('disabled', true);
						$(this).find('.submit-button').css({'display' : 'none' });
						$(this).find('.reset-button').css({'display' : 'none' });
 ...
 * @param		testing all 
 */
window.AdminPaperWork = function( obj ){
	
var dataPaperWorkData = new Ext.EventUrl( new Array( 'AdminPaperWork','index' )),
	MasterDataId = Ext.Cmp('DM_Id').getValue()
	
 $('#tab-bottom-activity-form').Spiner ({
	url  	: dataPaperWorkData.Apply(),
	method	: 'GET',
	param 	:{
		CustomerId : MasterDataId
	},
	complete : function( html ){
		$(html).css({ "height": "100%" });
		window.setTimeout( function(){
					$('.admpaperwork').each( function(){
						$(this).find('select').attr('disabled', false);
						$(this).find('input').attr('disabled', false);
						$(this).find('.button-adm').css({'display' : 'yes' });
						
					});
				}, 2000);
		//new window.DataDisAllowed();
	}
});
...
window.EventClose=function()
{
	...
	if( (Ext.DOM.initFunc.isCancel==true ) ) 
	{
		...
		Ext.DOM.UnsetFollowup( Ext.Cmp('DM_Id').getValue() );
 ...
 $('.boox').css({ 'width' : '75px' });	
  ...
  $('.tolong').each( function( item, val ){
	  ...
	  // jika jenis text area 
	   if ( $(val).is('textarea') ){
		  $(val).css({ 'width' : '250px', 'height' : '120px'});
	  }	  
  });
}
 /*
...
window.DataDisAllowed = function(){
	var DataDisAllowed = Ext.Cmp('DataDisAllowed').getValue();
	$('.disallowed').each(function(key, val ){
		var dataObjectID = $(val).attr('id');
		if( dataObjectID && DataDisAllowed == 1 ){
			Ext.Cmp(dataObjectID).disabled(true);
			$('.ui-widget-refresh-active').css({'display' : 'none' });
		}	
	});
}
...
// ---------- tab history ------------------------
  ...
  window.PageCallHistory({page : 0, orderby : "", type : ""});
  window.PaperWork({page : 0, orderby : "", type : ""});
  window.AdminPaperWork({page : 0, orderby : "", type : ""});
  window.EventCallRecording({page : 0, orderby : "", type : ""});
...
// get default last calling phone number by user 
	var CallLastNumber = Ext.Cmp('CallLastNumber').getValue();
	if( CallLastNumber ){	
		Ext.Cmp('CallingNumber').setValue(CallLastNumber);
	}
	
// disabled jika data sudah sale ya.
	window.DataDisAllowed();
