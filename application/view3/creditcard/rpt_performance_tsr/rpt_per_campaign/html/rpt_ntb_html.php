	...
	if ( $data_ntb != false ) {
		...
		foreach ( $data_ntb as $ky => $va ) {
			...
			foreach($va as $ke => $vl){
				...
				$totalcloseratakanan = $duals[$ky][$ke]['Duals']+$data_clos[$ky][$ke]['Closedeal']+$addonke1[$ky][$ke]['Closedeal']+$addonke2[$ky][$ke]['Closedeal'];
				...
				$row_data_ntb .= "<td>" . ($duals[$ky][$ke]['Duals']?$duals[$ky][$ke]['Duals']:0) . "</td>";
