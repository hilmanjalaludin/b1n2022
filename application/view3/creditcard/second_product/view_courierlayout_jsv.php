<script>
	...
	Ext.DOM.SaveLayout = function() {
		...
		Ext.Ajax({
			url: Ext.EventUrl(['SecondProduct', 'SaveCourier']).Apply(),
			method: 'POST',
			param: {
	...
	Ext.DOM.UpdateLayout = function() {
		...
		Ext.Ajax({
			...
			method: 'POST',
			param: {
				KurirCode: Ext.Cmp('CV_Data_Campaign_Id').getValue(),
				KurirDesc: Ext.Cmp('CV_Data_Campaign_Id').getValue(),
				flag: Ext.Cmp('CV_Data_Campaign_Id').getValue(),
				KurirID: Ext.Cmp('CV_Data_Campaign_Id').getValue(),
			},
			ERROR: function(fn) {
