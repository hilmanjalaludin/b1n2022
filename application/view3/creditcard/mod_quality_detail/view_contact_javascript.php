...
var dataRequired = Ext.Serialize('frmActivityData').Required( new Array('CallStatusId','CallResultId'));
 ...
 var dataConnUrl = Ext.EventUrl( new Array('ModSaveActivity/SaveQualityActivity'));
	 Ext.Ajax
 ({
...
window.EventAdd = function(){
	window.AdditionalPhone( Ext.Cmp('DM_Id').getValue() );
	return false;
}
...
window.disabledDataUserSale = function(){
	var callKategoryID = Ext.Cmp('CallStatusId').getValue();
	if( !callKategoryID.localeCompare(window.CONFIG.CALL_KATEGORY.CLOS) ){
		$('.after-user-sale').each( function(key, item ){
			Ext.Cmp(item).disabled(true);
		});
		// disabled button agar tidak bisa save .
		$(['.save', '.dial', '.hangup']).each(function( key, item ){
