 ...
 Ext.DOM.QualityCallHistory = function( obj )
{
   ...
   Ext.Ajax 
   ({
		url    : Ext.EventUrl(['ModCallHistory','PageCallMonQualityHistory']).Apply(),
		method : "GET",
		param  : {
			CustomerId 	: CustomerId,
			page 		: obj.page,
 ...
 */
 ...
 Ext.DOM.SaveAccurate =function()
{
	
 var params = new Array();
 var frmQualityActivity = Ext.Serialize('frmQualityActivity');
 if( !frmQualityActivity.Complete(['CustomerId','RemarksAccurate']) ){
	Ext.Msg("Form input not complete!").Info();
	return false;
 }
...
// ------------------------------------------------------------------------- 
 Ext.Ajax
 ({
	url 	: Ext.EventUrl(new Array("QtyApprovalInterest","SaveAccurate")).Apply(), 
	method 	: 'POST',
