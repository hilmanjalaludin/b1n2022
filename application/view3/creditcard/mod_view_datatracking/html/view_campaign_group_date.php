...
{ 
 ...
 {
	$CI  =& get_instance();
	$obj =& get_class_instance("M_DataTrackingReport");
...
//----------------- utilize data new ----------
 $UI->db->reset_select();
 $UI->db->select("
	count(distinct a.CustomerId) as data_new_utilize,
	date(a.CallHistoryCallDate) as tgl",FALSE);
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
 {
	$data[$rows['tgl']]['data_new_utilize'] += $rows['data_new_utilize'];
 } 

 //----------------- utilize data Recycle ----------
 
 $UI->db->reset_select();
 $UI->db->select("
	count(distinct a.CustomerId) as data_old_utilize,
	date(a.CallHistoryCallDate) as tgl", FALSE);
	
 $UI->db->from("t_gn_callhistory a FORCE INDEX(primary)");
 $UI->db->join("t_gn_customer b ","a.CustomerId=b.CustomerId", "LEFT");
 $UI->db->where("a.HistoryType", 0);
 $UI->db->where("(a.CallBeforeReasonId<>99 OR a.CallBeforeReasonId IS NULL OR a.CallBeforeReasonId <> 0 )", "", FALSE);
 $UI->db->where_in("b.CampaignId", $CampaignId);
 $UI->db->where("a.CallHistoryCallDate>='{$Out->get_value('start_date','StartDate')}'", "", false);
 $UI->db->where("a.CallHistoryCallDate<='{$Out->get_value('end_date','EndDate')}'", "", false);
 $UI->db->group_by(array("tgl"));
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
 {
	$data[$rows['tgl']]['data_old_utilize'] += $rows['data_old_utilize'];
 } 
...
// ---------- ambil data atemp pertiap tgl --------------
 ...
 $UI->db->reset_select();
 $UI->db->select("count(a.CallHistoryId) as data_size_atempt,DATE(a.CallHistoryCallDate) as tgl", 
FALSE);
...
// --- load status in history Hight db -----------------------------
 ...
 $UI->db->select(" 
	...
	(  select cgs.CallReasonCode
	   ...
	   and tgs.HistoryType = 0 ) as CallreasonIdCode", FALSE);
...
// ====================================== 
  echo "<tr>
		 ...
		 {
			$tot_pif_per_product 	= ( $call_data_content[$ProductId]['tot_nos_per_product'] ? $call_data_content[$ProductId]['tot_nos_per_product'] : 0);
			$tot_nos_per_product 	= ( $call_data_content[$ProductId]['tot_policy_per_product'] ? $call_data_content[$ProductId]['tot_policy_per_product'] : 0);
 ...
 $sum_tot_avg_talk_time_per_hour= ( $sum_tot_work_hours ? ($sum_tot_talk_time/($sum_tot_work_hours/$tot_const_perhours)): 0 );
 $sum_tot_avg_talk_time_per_tmr= ( $sum_tot_talk_time ? ($sum_tot_talk_time/ $sum_tot_tmr_login) : 0);
 $sum_tot_productivity_per_tmr_pif = (($sum_tot_tmr_login) ? ($sum_tot_acv_pif/$sum_tot_tmr_login/$sum_tot_work_days) : 0);
 $sum_tot_productivity_per_tmr_premi = (($sum_tot_tmr_login) ? ($sum_tot_acv_premi/$sum_tot_tmr_login/$sum_tot_work_days) : 0);
 
 $sum_tot_avg_atempt_per_tmr = (($sum_tot_tmr_login) ? ($sum_tot_attempt_call/$sum_tot_tmr_login/$sum_tot_work_days) : 0); 
 ...
 $sum_avg_data_productivity_anp_per_tmr = ( ($sum_tot_tmr_login AND $sum_tot_work_days ) ? ( $sum_tot_acv_premi_anp/$sum_tot_tmr_login/$sum_tot_work_days ) : 0);
 ...
 function showheaders()
{
	echo "<div class=\"center\">".
		 "<p class=\"normal font-size22\">Report Data Tracking - Campaign Group By Date</p>".
...
{
	$class =& get_class_instance("M_DataTrackingReport");
