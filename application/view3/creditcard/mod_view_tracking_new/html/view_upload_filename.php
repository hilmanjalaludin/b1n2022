 ...
 */	
 ...
 if( !function_exists('_select_report_upload_id') )
{ 
	function _select_report_upload_id()
 { 
	...
	$result_assoc = array();
	$result_valcamp = _select_report_campaign_value();
	
	// get data process 
	$sql = sprintf("select a.FTP_UploadId, a.FTP_UploadDateTs, a.FTP_UploadFilename 
					from t_gn_upload_report_ftp a 
					left join t_gn_customer_master b on a.FTP_UploadId=b.DM_UploadId  where a.FTP_Flags =1
					and b.DM_CampaignId IN(%s)
					group by a.FTP_UploadId", SetWhereIn($result_valcamp, true));
	
	//echo $sql;	
	$qry = $CI->db->query($sql);
	if( $qry && $qry->num_rows() > 0 ) 
	foreach( $qry->result_assoc() as $row ) {
		$result_assoc[$row['FTP_UploadId']]['file'] = $row['FTP_UploadFilename'];
		$result_assoc[$row['FTP_UploadId']]['date'] = $row['FTP_UploadDateTs'];
	}	
 ...
 */	
 ...
 if( !function_exists('_select_report_campaign_value') )
{ 
	function _select_report_campaign_value()
 {
	 
	$dataURI = UR();
	$result_data = array();//$dataURI->fields('CampaignId');
	$result_post = array();
	
	// select all campaign selected 
	$CI = &get_instance();
	$sql = sprintf("select a.CampaignId from t_gn_campaign a where a.CampaignStatusFlag = %d", 1);
	$qry = $CI->db->query($sql);
	if( $qry && $qry->num_rows() > 0 ) 
	foreach( $qry->result_assoc() as $row ){
		$result_data[$row['CampaignId']]= $row['CampaignId'];
	} 	
	
	// check data proces 
	$result_finder = false;
	$result_assoc = $dataURI->fields('CampaignId');
	if( is_array($result_assoc) and count($result_assoc) > 0 ) 
	foreach($result_assoc as $key => $val ){
		if( !strcmp( $val, 'all') ){
			$result_finder = true;
		} else {
			$result_post[$val] = $val; 
	...
	// then will check againts 
	//var_dump($result_finder);
	if( !$result_finder ){
		return (array)$result_post;
	}
...
// parameter object loacal data 
	$arr_rec_value 	 = _select_report_campaign_value();
	$arr_var_upload  = _select_report_upload_id();
...
// get data init freq_call per lead 
	$arr_val_data  = array();
	$sql = sprintf("SELECT count(*) as tot_init_call, b.DM_UploadId as UploadId,
				...
				WHERE a.HistoryType = 0 AND  b.DM_CampaignId IN(%s)
				
				GROUP BY UploadId", SetWhereIn( $arr_rec_value, true ) );
	...
	if( $qry && $qry->num_rows() > 0) 
	foreach( $qry->result_assoc() as $row )  {
		$arr_val_data[$row['UploadId']]['tot_init_call'] += $row['tot_init_call'];	
		$arr_val_data[$row['UploadId']]['tot_init_contacted'] += $row['tot_init_contacted'];	
		$arr_val_data[$row['UploadId']]['tot_init_notcontacted'] += $row['tot_init_notcontacted'];	
		$arr_val_data[$row['UploadId']]['tot_init_leads'] += $row['tot_init_leads'];
	}
...
// untuk mendapakan semua data dari masing2 campaign.
		 ...
		 foreach( $qry->result_assoc() as $row )
	{
		...
		$arr_val_data[$row['UploadId']]['data_size'] += $row['tot_size'];
		$arr_val_data[$row['UploadId']]['data_util'] += $row['tot_util'];
		
		
	// data followup 
		$arr_val_data[$row['UploadId']]['APMT'] += $row['APMT'];
		$arr_val_data[$row['UploadId']]['BSTO'] += $row['BSTO'];
		$arr_val_data[$row['UploadId']]['NPU']  += $row['NPU'];
		$arr_val_data[$row['UploadId']]['FOLW'] += $row['FOLW'];
		
	// data decline call 	
		$arr_val_data[$row['UploadId']]['FTM']  += $row['FTM'];
		$arr_val_data[$row['UploadId']]['NOAN'] += $row['NOAN'];
		$arr_val_data[$row['UploadId']]['OWAC'] += $row['OWAC'];
		$arr_val_data[$row['UploadId']]['OWBN'] += $row['OWBN'];
		$arr_val_data[$row['UploadId']]['PTM']  += $row['PTM'];
		$arr_val_data[$row['UploadId']]['REG']  += $row['REG'];
		$arr_val_data[$row['UploadId']]['RUF']  += $row['RUF'];
		$arr_val_data[$row['UploadId']]['VER']  += $row['VER'];
		$arr_val_data[$row['UploadId']]['ULIM'] += $row['ULIM'];
		$arr_val_data[$row['UploadId']]['UFML'] += $row['UFML'];
		$arr_val_data[$row['UploadId']]['UICM'] += $row['UICM'];
		$arr_val_data[$row['UploadId']]['PTS']  += $row['PTS'];
		$arr_val_data[$row['UploadId']]['NOEC'] += $row['NOEC'];
		$arr_val_data[$row['UploadId']]['AGE']  += $row['AGE'];
		$arr_val_data[$row['UploadId']]['IPT']  += $row['IPT'];
		$arr_val_data[$row['UploadId']]['NONPW']+= $row['NONPW'];
		
		
	// data not contacted 
	
		$arr_val_data[$row['UploadId']]['INVD'] += $row['INVD'];
		$arr_val_data[$row['UploadId']]['WRNO'] += $row['WRNO'];
		$arr_val_data[$row['UploadId']]['MOVE'] += $row['MOVE']; 
		
	// data not status < NSTS>	
		$arr_val_data[$row['UploadId']]['NSTS'] += $row['NSTS'];
	// data process 
		$arr_val_data[$row['UploadId']]['APRV'] += $row['APRV'];
		$arr_val_data[$row['UploadId']]['CLOS'] += $row['CLOS'];
		$arr_val_data[$row['UploadId']]['BLCK'] += $row['BLCK'];
		$arr_val_data[$row['UploadId']]['RDPC'] += $row['RDPC'];
		
		
		
	// data complete && Process di pisah .
	
		$arr_val_data[$row['UploadId']]['PROC'] += $row['PROC'];
		$arr_val_data[$row['UploadId']]['COMP'] += $row['COMP'];
	}
	...
	print("<table class=\"data\" border=1 style=\"border-collapse: collapse\">
		<tr>
			<td rowspan=\"2\"  class=\"head\">Upload Date</td> 
			<td rowspan=\"2\"  class=\"head\">File Name</td> 
...
// ambil data campaign -nya kemudian loping datanya.
	//debug($arr_var_upload);
  if( is_array($arr_var_upload)  ) 
	foreach( $arr_var_upload as $UploadId => $files )
  {
	$val_file = Objective($files);  
	$val_data = ( $arr_val_data[$UploadId] ? $arr_val_data[$UploadId] : null );	
...
// quality Process 
	$totDataQualityProcess =($APRV+$RDPC+$BLCK+$CLOS);
	$totDataPercentQualityProcess = ( $totDataUtil ?(($totDataQualityProcess/$totDataUtil) *100) : 0 );
	
// data Process adalah data yang sudah Sale secara Agent yang akan masuk ke 
// ke QA .

	//$totDataSaleProcess = $PROC;
	//$totDataSaleComplete = $COMP;
...
// call pada kurun waktu ackuan - nya campaign.
	...
	$totAvgFreqInitCall 	= ( $totDataUtil ? number_format(($totDataFreqInitCall / $totDataUtil) , 1 ): 0 );	
	 
	printf("%s", "<tr class=\"content\">");
			printf("<td class=\"content\" nowrap>%s</td>", $val_file->field('date','SetDateTime'));
			printf("<td class=\"content\" nowrap>%s</td>", $val_file->field('file', 'basename'));
	...
	// customize button data 	
	  ...
	  $totBootAvgFreqInitCall = ($totBootDataSize ? number_format(($totBootDataFreqInitCall / $totBootDataUtil) , 1 ): 0 ); 
	  
	  $totBootAvgFreqCallContacted = ($totBootDataUtil ? number_format(($totBootDataFreqContactedInitCall / $totBootDataUtil), 1 ): 0 );	
	  $totBootAvgFreqCallNotContacted = ($totBootDataUtil ? number_format(($totBootDataFreqNotContactedInitCall / $totBootDataUtil), 1 ): 0 );									
	...
	// ------- bootom test ------------------------------
		...
		printf("%s", "<tr>");
			printf("<td class=\"head\" colspan=\"2\" align=\"center\">%s</td>", "Summary");
...
// report tersebut .
	...
	printf("<div class=\"center\">
			<p class=\"normal font-size22\">Report Call Tracking - Summary By Upload File</p>
...
<html>
	<head>
		<title><?php echo title_header("Call Tracking - Summary By Upload File");?></title> 
