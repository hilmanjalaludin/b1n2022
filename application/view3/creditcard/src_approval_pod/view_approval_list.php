 ...
 if( !function_exists('_setEventPrint') )
{
	function _setEventPrint( $PodId ) 
	{
		...
		$arr_button = "&nbsp;";
		if( $Buttons->find_value('_ACT_APRV_') 
			&& $Buttons->find_value('_ACT_RJCT_'))
		{
			$arr_button= form()->formtoolbar ( array(  
					$Buttons->get_value('_ACT_APRV_')->get_value('Event') => array("class" => "fa fa-check-circle ui-widget-awesome-toolbar", "value" => $PodId, "label" => $Buttons->get_value('_ACT_APRV_')->get_value('Label')),
					$Buttons->get_value('_ACT_RJCT_')->get_value('Event') => array("class" => "fa fa-check-circle ui-widget-awesome-toolbar", "value" => $PodId, "label" => $Buttons->get_value('_ACT_RJCT_')->get_value('Label'))
				)
			);
		}
		
		else if( $Buttons->find_value('_ACT_APRV_') 
			&& !$Buttons->find_value('_ACT_RJCT_'))
		{
			$arr_button= form()->formtoolbar ( array(  
					$Buttons->get_value('_ACT_APRV_')->get_value('Event') => array("class" => "fa fa-check-circle ui-widget-awesome-toolbar", "value" => $PodId, "label" => $Buttons->get_value('_ACT_APRV_')->get_value('Label'))
...
if( $button->find_value('_DTL_TOOL_') ) {
   $pager->set_event_row_forbiden_click(array('PrintId'));
