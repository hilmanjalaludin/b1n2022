...
{
	$result_array = array();
	$sql = sprintf("select a.*, b.CampaignCode as campaign, cs.DM_QualityReasonKode from t_gn_customer_verification a 
			 inner join t_gn_campaign b on b.CampaignId = a.CV_Data_Campaign_Id
			 left join t_gn_customer_master cs on cs.DM_Id = a.CV_Data_CustId
...
// customerID 
 ...
 foreach( $qry->result_assoc() as $row ){
	...
	// $result_array[$kartu->field('CV_Data_Id')]['value']   = $kartu->field('CV_Data_MemberSince');
	$result_array[$kartu->field('CV_Data_Id')]['value']   = ($setup->field('SV_Data_ProductId') == 27) ? $kartu->field('CV_Data_CcExpired') : $kartu->field('CV_Data_MemberSince');
...
function pencarian_programdetail($ProgramId)
	...
	function pencarian_membersinceDb($ProgramId)
	{
		// SELECT * FROM t_lk_program_detail r
		// WHERE r.PRD_Data_Id=''

		$this->db->reset_select();
		$this->db->select('CV_Data_MemberSince');
		$this->db->from('t_gn_customer_verification');
		$this->db->where('CV_Data_Id', $ProgramId);
