...
// ----------- outbound only ---------------------------------
	...
	$this->db-> select("
			...
			g.Assign_Data_Id,  
			(SELECT IF( sum(a.duration) IS NULL, 0, sum(a.duration)) 
			FROM cc_recording a WHERE a.assignment_data=b.CustomerId ) as TalkTime", 
	FALSE);
	
	$this->db-> from("t_gn_assignment a ");
	$this->db-> join("t_gn_customer b ","a.CustomerId=b.CustomerId","INNER");
	$this->db-> join("tms_agent c", "b.SellerId=c.UserId","INNER");
	$this->db-> join("tms_agent d", "c.tl_id=d.UserId","LEFT");
	...
	$this->db-> join("t_gn_quality_assignment g ", "a.AssignId = g.Assign_Data_Id","LEFT");
	...
	$this->db->where("b.CallReasonId BETWEEN 44 AND 107", NULL );
	$this->db->where_not_in("b.CallReasonId", array("NULL"));
  ...
  if( _get_have_post("orderby") ){
	 ...
	 $this->db->order_by("a.AssignId", "ASC");
  }
...
// ----------------------------------------------------------------------------------------------
	$AssignId = (array)$out->get_array_value('AssignId');
	$QualityStaffId = $out->get_value('SPVId');
// -----------------------------------------------------------------------------
	
 if( count($AssignId) == 0  OR  count($QualityStaffId) == 0 ){
	 return FALSE;
 }	
...
// ----------------------- next process -----------------------------------
	...
	if( $jumlah_per_staff_quality ) 
 {
	...
	for( $user = 0;  $user < $jumlah_user; $user++)
	{
		...
		$arr_list_assign[$QualityStaffId] = array_slice($AssignId, $offset, $jumlah_per_staff_quality);
		$start++;
	}
...
// --- next process  ---------------------
	...
	$total_data = 0;
	foreach( $Assign_Data_Id AS $key => $AssignId )
	{
		$this->db->reset_select();
		...
		if( $this->db->affected_rows() > 0 ) {
			...
			$this->_setQualityStatusCustomer($AssignId, $Quality_Staff_Id);
			$total_data++;
		}
	}
...
/* private function set Data Reason ID On T_gn _customer with QA Status COde 505 .*/
 ...
 private function _setQualityStatusCustomer( $AssignId  = 0, $QualityStaffId = 0 )
{
...
// -------- next ------------------------------------------------------ 
 ...
 foreach( $List_data as $key => $rows ) {
	$row = new EUI_Object( $rows );
	$assign_array[$i] = $row->get_value('AssignId');
	$i++;
 }
...
if( $cond == true) { 
 ...
 $this->db->join("t_gn_assignment b ","a.Assign_Data_Id=b.AssignId", "LEFT");
...
$this->db->select("
 ...
 $this->db->from("t_gn_quality_assignment  a ");
 $this->db->join("t_gn_assignment b "," a.Assign_Data_Id=b.AssignId", "INNER");
