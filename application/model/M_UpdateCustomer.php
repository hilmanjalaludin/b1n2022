<?php
  ...
  */
class M_UpdateCustomer extends EUI_Model
{
...
// default fileter 
	...
	$this->EUI_Page->_setAnd("a.DM_ProductId", USAGE);	

//  post filter  --------------------------------------------------------------	
	$this->EUI_Page->_setAndCache("a.DM_CallCategoryId", 'UPD_call_category', 	TRUE);
	$this->EUI_Page->_setAndCache("a.DM_LastCategoryId", 'UPD_last_category', 	TRUE);
    $this->EUI_Page->_setAndCache("a.DM_SellerId",		 'UPD_user_agent', 		TRUE);
    $this->EUI_Page->_setBeginCache("a.DM_UpdatedTs",	 'UPD_call_start_date', TRUE);
    $this->EUI_Page->_setStopCache("a.DM_UpdatedTs", 	 'UPD_call_end_date', 	TRUE);
	$this->EUI_Page->_setFieldCache('LIKE', 			 'UPD1_filter_field', 	'UPD1_filter_value', TRUE);
    $this->EUI_Page->_setFieldCache('LIKE', 			 'UPD2_filter_field', 	'UPD2_filter_value', TRUE);
 ...
 function _select_row_master_detail( $MasterDataId=null)
{
 ...
 // data lai akan di ambil pake fungsi aja .
  ...
  $sql = sprintf("
  	select * , b.CampaignDesc as DM_CampaignName 
...
// atau approve
  if( $this->dataCOK->find_value('UserId') ){
	$this->db->where_not_in( 'a.DM_LastCategoryId', array(APRV,CLOS,YCOM,NCOM ));	
  }
 ...
 * @return [type]             [description]
 */	 
function _set_row_update_data( $URI = null  ){
	$COK =&CK();
	 
	$URI->add('CV_Data_UpdateTs', date('Y-m-d H:i:s'));
  
	// catat sebelum di input 
	$result_array = $URI->fetch_assoc();
	
	$writeloger = array(); 	
	$writeloger[] = sprintf("UPDATE DATA >> %s :\n", $COK->field('Username'));  
	if( is_array( $result_array ) ) 
	foreach( $result_array as $key => $val ){
		$writeloger[] = sprintf("%s:%s\r\n", $key, $val);	
	} 
	// set on loger data 
	writeloger( join('', $writeloger ));
	
	// check data process step by step 
	if( is_null( $URI ) OR !is_object( $URI ) ){
		return false;
	}
	
	// update di data recontest . simpan di history table 
	// untuk data verifikasi 
	$this->continue = 0;
	$sql = sprintf( "INSERT INTO t_gn_upload_refresh (  UR_Data_UploadId, UR_Data_FixID, UR_Data_Membal, UR_Data_AvailXD,
														UR_Data_AvailSS,  UR_Data_Block, UR_Data_Crelimit, UR_Data_CcExpired,
														UR_Data_UploadTs, UR_Data_UploadUser  ) 
							SELECT 	0 as UR_Data_UploadId, a.CV_Data_FixID as UR_Data_FixID,
									a.CV_Data_Membal as UR_Data_Membal, a.CV_Data_AvailXD as UR_Data_AvailXD,
									a.CV_Data_AvailSS as UR_Data_AvailSS, a.CV_Data_Block as UR_Data_Block,
									a.CV_Data_Crelimit  as UR_Data_Crelimit, a.CV_Data_CcExpired as UR_Data_CcExpired,
									NOW() as  UR_Data_UploadTs, '%s' as UR_Data_UploadUser
							FROM t_gn_customer_verification a 
							WHERE a.CV_Data_Id=%s", $COK->field('Username'), 
													$URI->field('CV_Data_Id'));
...
// go anywhere process 
 ...
 // jika kontinue gagal kembalikan data ke proces di 
 // kontroller 
 if( !$this->continue ){
	return false;	
 }
  
// then update data di "t_gn_customer_verification" 
 $this->continue = 0;
 
// that on this set OK process SIP . 
 $this->db->reset_write();
 $this->db->set('CV_Data_FixID', 	 $URI->field('CV_Data_FixID')); 				// => 5300864311W710093401
 $this->db->set('CV_Data_CardType',  $URI->field('CV_Data_CardType')); 				// => ME
 $this->db->set('CV_Data_CcExpired', $URI->field('CV_Data_CcExpired')); 			// => 0917
 $this->db->set('CV_Data_Penawaran', $URI->field('CV_Data_Penawaran')); 			// => 
 $this->db->set('CV_Data_Cycle', 	 $URI->field('CV_Data_Cycle')); 				// => 16
 $this->db->set('CV_Data_Block', 	 $URI->field('CV_Data_Block')); 				// => 0
 $this->db->set('CV_Data_OpenDate',  $URI->field('CV_Data_OpenDate','SetDate')); 	// => 21-09-2005
 $this->db->set('CV_Data_Crelimit',  $URI->field('CV_Data_Crelimit','SetNominal')); // => 25.000.000
 $this->db->set('CV_Data_Membal', 	 $URI->field('CV_Data_Membal', 'SetNominal')); 	// => 11.599.857
 $this->db->set('CV_Data_AvailXD', 	 $URI->field('CV_Data_AvailXD','SetNominal')); 	// => 6.700.072
 $this->db->set('CV_Data_AvailSS', 	 $URI->field('CV_Data_AvailSS', 'SetNominal')); // => 6.700.072
 
 // cross data 
 $this->db->set('CV_Data_Dob',		 $URI->field('DM_Dob', 'SetDate'));
 $this->db->set('CV_Data_MotherName',$URI->field('DM_MotherName','strtoupper'));
 $this->db->set('CV_Data_RzipCode',	 $URI->field('DM_ZipCode'));
 $this->db->set('CV_Data_LzipCode',	 $URI->field('CV_Data_LzipCode'));
 $this->db->set('CV_Data_UpdateTs',  $URI->field('CV_Data_UpdateTs'));
 
 // where kondisi data 
 $this->db->where('CV_Data_Id', 	$URI->Field('CV_Data_Id'));
 
 
// jika process berhasil maka masukan nilainya continue -n ay 
// untuk process selanjutnya.
 ...
 // update data customer process OK 
 $this->db->reset_write();
 $this->db->set('DM_Custno', $URI->field('DM_Custno')); 
 $this->db->set('DM_FirstName', $URI->field('DM_FirstName','strtoupper')); 
 $this->db->set('DM_Dob', $URI->field('DM_Dob', 'SetDate')); 
 $this->db->set('DM_MotherName', $URI->field('DM_MotherName','strtoupper')); 
 $this->db->set('DM_ZipCode', $URI->field('DM_ZipCode')); 
 $this->db->set('DM_OfficeZipCode', $URI->field('CV_Data_LzipCode')); 
 $this->db->set('DM_HomePhoneNum', $URI->field('DM_HomePhoneNum')); 
 $this->db->set('DM_MobilePhoneNum', $URI->field('DM_MobilePhoneNum')); 
 $this->db->set('DM_OtherPhoneNum', $URI->field('DM_OtherPhoneNum')); 
 
 // CROSS DATA 
 
 $this->db->set('DM_CrLimit', $URI->field('CV_Data_Crelimit','SetNominal')); 
 $this->db->set('DM_CcTypeName', $URI->field('CV_Data_CardType')); 
 $this->db->set('DM_DataOpenDate', $URI->field('CV_Data_OpenDate','SetDate'));  
 $this->db->set('DM_DataMembal', $URI->field('CV_Data_Membal', 'SetNominal')); 	 
 $this->db->set('DM_DataAvailXD',$URI->field('CV_Data_AvailXD','SetNominal')); 	 
 $this->db->set('DM_DataAvailSS', $URI->field('CV_Data_AvailSS', 'SetNominal'));  
 $this->db->set('DM_CcExpired', $URI->field('CV_Data_CcExpired')); 		 
 $this->db->set('DM_DataPenawaran', $URI->field('CV_Data_Penawaran')); 		 
 $this->db->set('DM_DataCycle', $URI->field('CV_Data_Cycle')); 
 $this->db->set('DM_DataBlock', $URI->field('CV_Data_Block'));  
 $this->db->where('DM_Id', $URI->field('DM_Id'));
