<?php
class M_CtiAgentGroup extends EUI_Model
{
 ...
 */
 ...
 function M_CtiAgentGroup()
 {
	if( is_null(self::$meta) )
	{
		self::$meta = 'cc_agent_group';
	}
 }
...
function _getReasonTypeData( $id = NULL )
{
	$this -> db -> select('*');
...
function _get_default()
{
	...
	{
		$flt .=" AND ( 
				a.code LIKE '%{$this->URI->_get_post('keywords')}%'  
				OR a.description LIKE '%{$this->URI->_get_post('keywords')}%'  
				OR a.direction LIKE '%{$this->URI->_get_post('keywords')}%'  
				OR a.hunting_number LIKE '%{$this->URI->_get_post('keywords')}%'  
				OR a.group_type LIKE '%{$this->URI->_get_post('keywords')}%'  
				OR a.overflow_group LIKE '%{$this->URI->_get_post('keywords')}%'  
				OR a.autoacw LIKE '%{$this->URI->_get_post('keywords')}%'  
			)";
	}				
