<?php
class M_CtiCCSetting extends EUI_Model
{
 ...
 */
 ...
 function M_CtiCCSetting()
 {
	if( is_null(self::$meta) ) {
		self::$meta = 'cc_settings';
		self::$join = array();
	}
 }
...
function _getComponent()
{
	return array
	( 
		'combo'   => array('NULL_STRINF'),
		'input'   => array('instance_id', 'set_modul', 'set_name', 'set_type','set_value','set_comment'),
		'primary' => array('id')
	);
}
...
function _get_default()
{
	...
	{
		$flt .=" AND ( 
				a.instance_id LIKE '%{$this->URI->_get_post('keywords')}%'  
				OR a.set_modul LIKE '%{$this->URI->_get_post('keywords')}%'  
