...
class M_Balcon extends EUI_Model  {
...
//   var_dump($this->dataURI->field('TX_Usg_ProgramData'));
//   die;

 if(!is_object( $this->dataURI ) ){
	 return false;
  };
//  var_dump($this->dataCOK = CK());
//  die;
...
// untuk mempermudah . =======================================================
 ...
 $this->dataURI->add('TX_Usg_Tigabulan',	$this->dataURI->field('TX_Usg_Tigabulan'));
 ...
 $this->dataURI->add('TX_Usg_ProgramValue', $this->dataURI->field('TX_Usg_ProgramData','strtoupper') );
 $this->dataURI->add('TX_Usg_Tigabulan',    $this->dataURI->field('TX_Usg_Tigabulan','strtoupper') );
...
//  $cust= $this->db->select('t_gn_frm_balcon');
// 		$this->db->where('TX_Usg_CustId',$this->dataURI->field('TX_Usg_CustId'));
...
// }		
 ...
 $this->db->set('TX_Usg_ProgramData',	$this->dataURI->field('PRD_Data_Id'));
 ...
 $this->db->set('TX_Usg_Tenor',			$this->dataURI->field('TX_Usg_Tenors'));
 ...
 $this->db->set('TX_Usg_Tigabulan',		$this->dataURI->field('TX_Usg_Tigabulan'));
 if($this->dataURI->field('TX_Usg_TransSeq')){
	$this->db->set('TX_Usg_TransSeq',		$this->dataURI->field('TX_Usg_TransSeq'));
 }
 
 // insert data untuk pertma kalinya
 $this->db->insert('t_gn_frm_balcon');
 ...
 if( $this->db->affected_rows() > 0 ){
	...
	$sql = sprintf("UPDATE t_gn_frm_balcon SET TX_Usg_TransSeq = %s, TX_Usg_TransId = '%s' 
 ...
 // berikut ini.
  ...
  // where on duplicate 
	...
	$this->db->where('TX_Usg_TransSeq', $this->dataURI->field('TX_Usg_TransSeq'));
	$this->db->update('t_gn_frm_balcon');
...
// query data for selector query && Process SIP 	
	...
	$this->db->where('a.PRD_Data_Master', 'BALCON');
