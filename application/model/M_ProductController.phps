	...
	*/
	...
	public function _get_row_ntb ( $CustomerNumber = "" ) {
		...
		if ( !empty($CustomerNumber) ) {
			...
			$this->db->join("t_gn_customer_master c" , "a.TR_CustomerNumber=c.DM_Custno" , "INNER");
	...
	public function _get_row_addon ( $CustomerNumber = "" ) {
		...
		if ( !empty($CustomerNumber) ) {
			...
			$this->db->select("b.*");
			$this->db->from("t_gn_frm_transaction_addon a");
			$this->db->join("t_gn_frm_addon b" , "a.TR_Id=b.ADDON_TransId" , "INNER");
			$this->db->where("ADDON_TypeProduct" , 0);
			$this->db->where("TypeProduct_Id" , 0);
	...
	public function _save_ntb ( $out = "" ) {
		...
		if ( is_object( $out ) and _have_get_session("UserId") ) {
			...
			$DC_Dual_Card_Limit 			     = $out->get_value("DM_CcLimit");
	...
	public function _save_addon_addon ( $req = "" ) {
		$call_back = array( "success" => 0 );
		if ( is_object( $req ) ) {
			$ADDON_Nama_Kartu 	   = $req->get_value("ADDON_Nama_Kartu_".$req->get_value("FRM_ADDON_Ke"));
			$ADDON_Hubungan 	   = $req->get_value("ADDON_Hubungan_".$req->get_value("FRM_ADDON_Ke"));
			$ADDON_Umur 		   = $req->get_value("ADDON_Umur_".$req->get_value("FRM_ADDON_Ke"));
			$ADDON_DOB             = $req->get_value("ADDON_DOB_".$req->get_value("FRM_ADDON_Ke"));
			$ADDON_Jenis_Kartu     = $req->get_value("ADDON_Jenis_Kartu_".$req->get_value("FRM_ADDON_Ke"));
			$ADDON_Jenis_Kelamin   = $req->get_value("ADDON_Jenis_Kelamin_".$req->get_value("FRM_ADDON_Ke"));
			// $ADDON_CustNum		   = $req->get_value("CustomerNumber_".$req->get_value("CustomerNumber"));
			$ADDON_CustNum 	       = $req->get_value("CustomerNumber");
			$ADDON_No_Hp 		   = $req->get_value("ADDON_No_Hp_".$req->get_value("FRM_ADDON_Ke"));
			$FRM_ADDON_Id 		   = $req->get_value("FRM_ADDON_Id");
			$FRM_ADDON_Ke		   = $req->get_value("FRM_ADDON_Ke");
			...
			$this->db->set("DB_Alamat_Krim_1" , $addontrans['DB_Alamat_Krim_1']);
			$this->db->set("DB_Alamat_Krim_2" , $addontrans['DB_Alamat_Krim_2']);
			$this->db->set("DB_Alamat_Krim_3" , $addontrans['DB_Alamat_Krim_3']);
			$this->db->set("DB_Alamat_Krim_4" , $addontrans['DB_Alamat_Krim_4']);
			$this->db->set("DB_Kota" , $addontrans['DB_Kota']);
			$this->db->set("DB_Kode_Pos" , $addontrans['DB_Kode_Pos']);
			$this->db->set("CreatedTs" , date("Y-m-d H:i:s"));
			$this->db->set("DB_Home_Phone" , $addontrans['DB_Home_Phone']);
			$this->db->set("DB_Mobil_Phone" , $addontrans['DB_Mobil_Phone']);
			$this->db->set("DB_Office_Phone" , $addontrans['DB_Office_Phone']);
			
			$this->db->duplicate("DB_Alamat_Krim_1" , $addontrans['DB_Alamat_Krim_1']);
			$this->db->duplicate("DB_Alamat_Krim_2" , $addontrans['DB_Alamat_Krim_2']);
			$this->db->duplicate("DB_Alamat_Krim_3" , $addontrans['DB_Alamat_Krim_3']);
			$this->db->duplicate("DB_Alamat_Krim_4" , $addontrans['DB_Alamat_Krim_4']);
			$this->db->duplicate("DB_Kota" , $addontrans['DB_Kota']);
			$this->db->duplicate("DB_Kode_Pos" , $addontrans['DB_Kode_Pos']);
			$this->db->duplicate("DB_Home_Phone" , $addontrans['DB_Home_Phone']);
			$this->db->duplicate("DB_Mobil_Phone" , $addontrans['DB_Mobil_Phone']);
			$this->db->duplicate("DB_Office_Phone" , $addontrans['DB_Office_Phone']);
			$this->db->duplicate("ADDON_Nama_Kartu" , $ADDON_Nama_Kartu);
			...
			$this->db->duplicate("ADDON_TransId" , $FRM_ADDON_Id);
			$this->db->duplicate("ADDON_Seq" , $FRM_ADDON_Ke);
			$this->db->duplicate("ADDON_No_Hp" , $ADDON_No_Hp);
			$this->db->duplicate("ADDON_TypeProduct" , 0);
			$this->db->duplicate("Agent_Code" , $this->user->get_value("Agent"));
			$this->db->duplicate("MGR_Code" , $this->user->get_value("Mgr"));
			$this->db->duplicate("SPV_Code" , $this->user->get_value("Spv"));
