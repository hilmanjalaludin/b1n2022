...
/**
 ...
 function _get_default() 
 {
	...
	$this->EUI_Page->_setFrom("t_gn_product_prefixnumber a");
...
/**
 ...
 {
	$datas = array();
	$sql = "select a.ProductId, a.ProductName FROM t_gn_product_master a";
...
function _getPrefixId($ProductId=0 )
{
	...
	$this -> db->from('t_gn_product_prefixnumber a');
		...
		$this -> URI->_get_array_post('PrefixId') as  $key => $PrefixId ) 
	{
		...
		if( $rows = $this ->db -> get() -> result_first_assoc() )
		{
			...
			if( $this -> db->delete('t_gn_formlayout') )
			{
				...
				if( $this -> db->delete('t_gn_product_prefixnumber') )
				{
					$_conds++;
				}
			}
 ...
 function _set_save_prefix_number( $_prefix_post=null )
 {
	...
	if(!is_null($_prefix_post) )
	{
		...
		if( !is_null($_get_chars) )
		{
			...
			$this->db->insert("t_gn_product_prefixnumber");
...
function _setUpdate( $_prefix_post = null )
{
	...
	if( $this -> db ->update("t_gn_product_prefixnumber",
	array
	(
