...
class M_ReportQa extends EUI_Model {
	...
	/**
	 * Get first Monitoring 
	 */
	
	public function FirstMonitoring ( $CustomerId = "" , $fetch = "" ) {
		$CustomerId = (int)$CustomerId;
		$query = "select 
				min(a.CallHistoryCreatedTs) as MinDate , 
				a.CustomerId , 
				a.ApprovalStatusId as ApproveStatus , 
				a.CallHistoryNotes as Remarks
				from t_gn_callhistory a 
				where a.CustomerId='$CustomerId'
				and a.ApprovalStatusId not in(9,99);";
		$sql = $this->db->query($query);
		if ( $sql == true AND $sql->num_rows() > 0 ) {
			$r = $sql->row();
			return $r->{$fetch};
		} else {
			return "error";
	...
	public function ReportperQa ( $status = "" , $idQa = "" , $startdate = "" , $enddate = "" , $statperformance = '' ) {
		if ( $statperformance == 'ALL' ) {
			...
			$statperformances = "group by date_format(a.Assign_Create_Ts , '%d-%m-%Y')";
		}
		...
		if ( $status == "getqa" ) {
			...
			if ( $startdate == $enddate ) {
				$datesbetween = "date_format(a.Assign_Create_Ts,'%d-%m-%Y')='$startdate'";
			} else {
				$datesbetween = "a.Assign_Create_Ts between STR_TO_DATE('$startdate','%d-%m-%Y') AND STR_TO_DATE('$enddate','%d-%m-%Y')";
			}
			
			$performanceperqa = "
			select
				b.CustomerId as CustomerId , 
				count( distinct b.CustomerId ) as totalProduction , 
				count( distinct if( b.CallReasonQue != 9 , b.CustomerId , '' ) )-1 as totalAchievementPif , 
				'' as AproveStatus ,
				date_format(a.Assign_Create_Ts , '%d-%m-%Y') as Updates ,
				c.full_name as NamaQa , 
				sum((
					select 
					sum(cr.duration) from cc_recording cr 
					where cr.assignment_data=b.CustomerId 
				)) as totalDuration ,
					
				count(distinct if ( b.CallReasonQue in( 15,16 ) , b.CustomerId  , '' ))-1 as Verified , 
				count(distinct if ( b.CallReasonQue in( 10,11,12,17,19,20,21 ) , b.CustomerId   , '' ))-1 as Suspend  ,
				count(distinct if ( b.CallReasonQue in( 3,13,14,18 ) , b.CustomerId   , '' ))-1 as Declined , 
				count(distinct if ( b.CallReasonQue in( 9 ) , b.CustomerId   , '' ))-1 as Pending 
				
				from t_gn_quality_assignment a
				inner join t_gn_customer b on a.Assign_Data_Id=b.CustomerId
				inner join tms_agent c on a.Quality_Staff_Id=c.UserId
				where $datesbetween
				and c.UserId='$idQa'
			...
			/**$performanceperqa = "select  
				...
				date_format(a.QA_UpdateTs , '%d-%m-%Y') as Updates ,
				c.full_name as NamaQa , 
				sum((
					select 
					sum(cr.duration) from cc_recording cr 
				...
				count(distinct if ( a.CallReasonQue in( 1,15,16 ) , a.CustomerId  , '' )) as Verified , 
				count(distinct if ( a.CallReasonQue in( 10,11,12,17,19,20,21 ) , a.CustomerId  , '' )) as Suspend  ,
				count(distinct if ( a.CallReasonQue in( 3,13,14,18 ) , a.CustomerId  , '' )) as Declined , 
				count(distinct if ( a.CallReasonQue in( 9 ) , a.CustomerId  , '' )) as Pending 
	...
	public function getQaActivity ( $startdate = '' , $enddate = '' , $status = '' , $statperform = '' , $idQa = "" ) {
		...
		*/
			$getactivityqa = "
				...
				as StartTime ,
				if( max(date_format(c.end_time,'%H:%i:%s')) IS NULL , 'N/A' , max(date_format(b.ActivityDate,'%H:%i:%s')) )
				as EndTime , 
	 ...
	 * @return [type]            [description]
	 */
	public function getSummaryQa ( $get = "" , $dates = "" , $enddates = "" , $statusapprove = "" ,  $getsummary = "" , $idagent = "" ) {
		...
		$idagent = (int)$idagent;
		if ( $idagent != "" ) {
 			$getall_idagent = " and d.SellerId='$idagent'";
 			$new_idagent = " and a.SellerId='$idagent'";
 			$production_idagent = "and c.SellerId='$idagent'";
 			$passqa_idagent = " and c.SellerId='$idagent'";

 			$getall_summary = " and d.SellerId in($idagent)";
 			$new_summary = " and a.SellerId in($idagent)";
 			$production_summary = "and c.SellerId in($idagent)";
 			$passqa_summary = " and c.SellerId in($idagent)";
		...
		if ( $getsummary == 1 ) {
			if ( $dates == $enddates ) {
				$getall 	= "date_format(c.PolicySalesDate,'%d-%m-%Y')='$dates' and d.CampaignId not in(19,20,21) and d.CallReasonId in(35,36,37) $getall_summary";
				$new 		= "date_format(c.PolicySalesDate,'%d-%m-%Y')='$dates' and a.CampaignId not in(19,20,21) and a.CallReasonId in(35,36,37) $new_summary";
				$production = "date_format(b.PolicySalesDate,'%d-%m-%Y')='$dates' and c.CampaignId not in(19,20,21) and c.CallReasonId in(35,36,37) $production_summary";
				$passqa 	= "date_format(b.PolicySalesDate,'%d-%m-%Y')='$dates' and c.CampaignId not in(19,20,21) and c.CallReasonId in(35,36,37) $passqa_summary";
				
			} else {
				$getall 	= "c.PolicySalesDate between '$dates_years' AND '$enddates_years' and d.CampaignId not in(19,20,21) and d.CallReasonId in(35,36,37) $getall_summary";
				$new 		= "c.PolicySalesDate between '$dates_years' AND '$enddates_years' and a.CampaignId not in(19,20,21) and a.CallReasonId in(35,36,37) $new_summary";
				$production = "b.PolicySalesDate between '$dates_years' AND '$enddates_years' and c.CampaignId not in(19,20,21) and c.CallReasonId in(35,36,37) $production_summary";
				$passqa 	= "b.PolicySalesDate between '$dates_years' AND '$enddates_years' and c.CampaignId not in(19,20,21) and c.CallReasonId in(35,36,37) $passqa_summary";
			}
			
		} else {
			$getall = "date_format(c.PolicySalesDate,'%d-%m-%Y')='$dates' and d.CampaignId not in(19,20,21) and d.CallReasonId in(35,36,37) $getall_idagent";
			
			$new 	= "date_format(c.PolicySalesDate,'%d-%m-%Y')='$dates' and a.CampaignId not in(19,20,21) and a.CallReasonId in(35,36,37) $new_idagent";
	...
	public function getPassQa ( $date = "" , $get = "" , $enddate = "" , $getsummary = ""  , $iduser = "" , $reason = '' , $dump = '' ) {
		...
		if ( $enddate != "" ) {
			$enddatesplusone = $enddate[1]+1;
			$enddate = explode( "-" , $enddate );
			$enddate = $enddate[2]."-".$enddatesplusone."-".$enddate[0];
		}
		...
		if ( $getsummary == 1 ) {
			...
			$getsummary = "c.PolicySalesDate between STR_TO_DATE('$date','%d-%m-%Y') AND STR_TO_DATE('$enddate','%d-%m-%Y')";
			// between STR_TO_DATE('$dates','%d-%m-%Y') AND STR_TO_DATE('$enddates','%d-%m-%Y')
		} else {
			if ( $iduser != "" ) {
			...
			$getsummary = "date_format(c.PolicySalesDate,'%Y-%m-%d')='$date' ";
		}
		...
		**/
		...
		if ( is_array( $getdate ) ) {
			...
			if ( strpos($enddate , '-') !== false ) {
				$enddate = explode( '-' , $enddate );
				$dates = (int)$enddate[0]+1;
				$months = $enddate[1];
				$years = $enddate[2];
				$enddate = $dates . "-" . $months . "-" . $years ; 
			}
			
			
			if ( $startdate == $enddate ) {
				$date = "date_format(b.PolicySalesDate,'%d-%m-%Y')='$startdate'";
			} else {
				// STR_TO_DATE('$dates','%d-%m-%Y') AND STR_TO_DATE('$enddates','%d-%m-%Y')
				$date = "b.PolicySalesDate between STR_TO_DATE('$startdate','%d-%m-%Y') AND STR_TO_DATE('$enddate','%d-%m-%Y') ";
			}
		} else {
			$date = "date_format(b.PolicySalesDate,'%d-%m-%Y')='$getdate'";
		} 
		...
		$getDetailSummaryQa = "select 
			...
			count(distinct if (c.CallReasonQue=15 , c.CustomerId , 1)) as TotVerifiedSelling ,
			count(distinct if (c.CallReasonQue=16 , c.CustomerId , 1)) as TotVerifiedReconfirm , 
			count(distinct if (c.CallReasonQue=11 , c.CustomerId , 1)) as TotSuspendDataEdit ,
			count(distinct if (c.CallReasonQue=12 , c.CustomerId , 1)) as TotSuspendSelling , 
			count(distinct if (c.CallReasonQue=14 , c.CustomerId , 1)) as TotDeclineConfirm ,
			count(distinct if (c.CallReasonQue=21 , c.CustomerId , 1)) as TotSuspendDataSellingConfirm ,
			count(distinct if (c.CallReasonQue=20 , c.CustomerId , 1)) as SuspendDataSellingPending , 
			count(distinct if (c.CallReasonQue=18 , c.CustomerId , 1)) as TotDeclineCancel ,
			count(distinct if (c.CallReasonQue=17 , c.CustomerId , 1)) as TotSuspendStill ,
			...
			left join tms_agent cv on h.spv_id=cv.UserId
	...
	public function getCallMon ( $startdate = "" , $enddate = "" ) {
		if ( $startdate == $enddate ) {
	 	...
	 	$getCallMon = "
			...
			c.SellerId as idAgent , 
			c.CallReasonQue as ReasonStatus , 
			...
			sum(a.PolicyPremi) as TotalPremi ,
			d.pointoferror as POE ,
			...
			if(c.CallReasonQue in(15,16) , 1 , 0 ) as StatusApproved , 
			l.PayerCity as Kota ,
	...
	public function getRemarks ( $idcustomer = "" , $reasonstatus = "" , $idagent = "" ) {
		if ( $idcustomer == "" OR $reasonstatus == "" ) {
			return " TIDAK ADA ";
		} else {
			if ( $idagent == "" ) {
				$idagents = "";
			} else {
				$idagents = "and a.CreatedById='$idagent'";
			}
			
			$getremarks = "
				select * from t_gn_callhistory a 
				where a.CustomerId='$idcustomer' 
				and a.ApprovalStatusId='$reasonstatus' 
				$idagents
			";
			$remarks = $this->db->query( $getremarks );
			if ( $remarks == true and $remarks->num_rows() > 0 ) {
				$rm = $remarks->row();
				return $rm->CallHistoryNotes;
			} else {
				return ' TIDAK ADA ROW ';
			}
		}
