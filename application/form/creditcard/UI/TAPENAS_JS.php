...
var  TAPENAS = function(){}, verifyDataJsonServer = {}, verifyKartuJsonServer = {};
...
// ambil dan proces counter disini ya .	
	...
	var rowDataJsonClientSisaCounter = ( rowDataJsonClientCounter-rowDataJsonClientFailed );
	 if( rowDataJsonClientFailed >= rowDataJsonClientCounter ){
		 verifyDataJsonServer.process+=1;
		 Ext.Msg(window.sprintf("Verifikasi Data \"%s\" ", rowDataJsonClientLabel )).Failed();
...
// process data loping 	
	var rowDataVerificationSuccess = 0,
		...
		//console.log(rowFieldDataProcess);
// jika input kosong 	
...
/**
  ...
  * @return [type]             [description]
  */
window.ActionPaperWork = function( ProductName, Program ){
// open window extra dana form 	with diffrent name row 
	var windowName = window.sprintf("%s%s", ProductName, Program);
	//alert("test");
	
// get customer ID from Cross ID .	
	var TapenasId 	 = ProductName
		CampaignName = Ext.Cmp('CampaignName').getValue(),
		CustomerId    = Ext.Cmp('CustomerId').getValue(),
		ProductId    = Ext.Cmp('ProductId').getValue(),
		ProductName  = Ext.Cmp('ProductName').getValue()
		dataURL  	 = Ext.EventUrl( new Array('ProductController', 'FormTapenas', ProductName, Program) ),
